...
void export_polygon_2(py::module_& m) {
  ...
  pgn_c.def(py::init<>())
    ...
    .def("bbox", &Pgn::bbox)
    .def("__getitem__",
         static_cast<const Point_2&(Pgn::*)(std::size_t)const>(&Pgn::operator[]))
  ...
  add_iterator<Vci, Vci, const Point_2&>("Vertex_iterator", pgn_c);
  pgn_c.def("vertices",
            [] (const Pgn& pgn)
            { return make_iterator(pgn.vertices_begin(), pgn.vertices_end()); },
            py::keep_alive<0, 1>());

  add_insertion(pgn_c, "__str__");
  add_insertion(pgn_c, "__repr__");

  bind_copy_iterator<CopyIterator<Eci>>(m, "Polygon_edges_iterator");
}
