...
void export_arr_algebraic_segment_traits_2(py::module_& m) {
  ...
  if (! add_attr<Integer>(m, "Integer")) {
    py::class_<Integer> integer_c(m, "Integer");  // no docstring available
  ...
  if (! add_attr<Ar>(m, "Algebraic_real_1")) {
    py::class_<Ar> ar1_c(m, "Algebraic_real_1");  // no docstring available
    ar1_c.def(py::init<>())
      .def(py::init<Ar&>())
      .def(py::init_implicit<int>())
  ...
  if (! add_attr<aos2::Bound>(m, "Bound")) {
    py::class_<aos2::Bound> bound_c(m, "Bound");  // no docstring available
    bound_c.def(py::init<>())
  ...
  py::class_<Gt> traits_c(m, "Arr_algebraic_segment_traits",
                          doc::CGAL_Arr_algebraic_segment_traits_2__class__);
  struct Concepts {
  ...
  // cv_c already has class doc from export_AosTraits_2
  cv_c.def("polynomial_2", &Cv::polynomial_2,
           doc::CGAL_Arr_algebraic_segment_traits_2_Curve_2_polynomial)
    ;
