 ...
 */
bool do_intersect_polylines(const std::vector<Point_3>& polyline1, const std::vector<Point_3>& polyline2) {
...
bool do_intersect_meshes(const PolygonMesh& pm1,
...
auto intersecting_meshes(const std::vector<PolygonMesh>& range,
  ...
  PMP::intersecting_meshes(range, std::back_inserter(result), internal::parse_pmp_np<PolygonMesh>(np));
...
auto self_intersections(const PolygonMesh& pm, const py::dict& np = py::dict()) {
  ...
  PMP::self_intersections(pm, std::back_inserter(result), internal::parse_pmp_np<PolygonMesh>(np)
                          // .vertex_point_map(vpm)
  );
...
//
template <typename PolygonMesh>
auto self_intersections_faces(const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& face_range,
  ...
  std::vector<std::pair<Fd, Fd>> result;
  PMP::self_intersections(face_range, pm, std::back_inserter(result), internal::parse_pmp_np<PolygonMesh>(np)
                          // .vertex_point_map(vpm)
  );
  return result;
}
...
auto triangle_soup_self_intersections(const std::vector<Point_3>& points,
                                      ...
                                      const py::dict& np = py::dict()) {
  std::vector<std::pair<std::size_t, std::size_t>> result;
  PMP::triangle_soup_self_intersections(points, triangles, std::back_inserter(result));
  return result;
}
...
//
template <typename PolygonMesh>
bool does_self_intersect_faces(
  ...
  return PMP::does_self_intersect(face_range, pm, internal::parse_pmp_np<PolygonMesh>(np)
                                  // .vertex_point_map(get_vertex_point_map(pm, np))
  );
}
...
void corefine(PolygonMesh& tm1, PolygonMesh& tm2, const py::dict& np1 = py::dict(), const py::dict& np2 = py::dict()) {
  ...
  auto eicm1 = get_edge_prop_map<PolygonMesh, bool>(
      tm1, "INTERNAL_MAP0", np1.contains("edge_is_constrained_map") ? np1["edge_internal_map"] : py::none());
  auto eicm2 = get_edge_prop_map<PolygonMesh, bool>(
      tm2, "INTERNAL_MAP1", np2.contains("edge_is_constrained_map") ? np2["edge_internal_map"] : py::none());
  ...
  if (!np1.contains("edge_is_constrained_map"))
    tm1.remove_property_map(eicm1);
  if (!np2.contains("edge_is_constrained_map"))
    tm2.remove_property_map(eicm2);
#endif  // CGALPY_PMP_POLYGONAL_MESH == 1
}

//
template <typename PolygonMesh, typename FaceComponentMap>
auto connected_components_map(PolygonMesh& pm, FaceComponentMap& fccmap, const py::dict& np = py::dict()) {
  using Pm = PolygonMesh;
  ...
  if (np.contains("face_index_map")) {
    ...
    retv = PMP::connected_components(
        pm, fccmap, internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_index_map(fim));
  } else {
    retv = PMP::connected_components(pm, fccmap, internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm));
  }
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
...
auto keep_connected_components_map(PolygonMesh& pm,
  ...
  if (np.contains("vertex_index_map")) {
    auto vim = get_vertex_prop_map<PolygonMesh, std::size_t>(
        pm, "INTERNAL_MAP0", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    PMP::keep_connected_components(pm, components_to_keep, fccmap,
                                   internal::parse_pmp_np<PolygonMesh>(np).vertex_index_map(vim));
  } else {
    PMP::keep_connected_components(pm, components_to_keep, fccmap, internal::parse_pmp_np<PolygonMesh>(np));
  }
}

template <typename PolygonMesh>
auto keep_connected_components(
    PolygonMesh& pm,
    const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& components_to_keep,
    const py::dict& np = py::dict()) {
  using Pm = PolygonMesh;
  using Gt = boost::graph_traits<Pm>;
  using Fd = typename Gt::face_descriptor;
  ...
  if (vimap && fimap) {
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
        pm, "INTERNAL_MAP1", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    auto fim = get_face_prop_map<Pm, std::size_t>(pm, "INTERNAL_MAP2",
                                                  np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    PMP::keep_connected_components(
        pm, components_to_keep,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).vertex_index_map(vim).face_index_map(
            fim));
  } else if (vimap) {
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
        pm, "INTERNAL_MAP1", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    PMP::keep_connected_components(
        pm, components_to_keep,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).vertex_index_map(vim));
  } else if (fimap) {
    auto fim = get_face_prop_map<Pm, std::size_t>(pm, "INTERNAL_MAP1",
                                                  np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    PMP::keep_connected_components(
        pm, components_to_keep,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_index_map(fim));
  } else {
    PMP::keep_connected_components(pm, components_to_keep, internal::parse_pmp_np<PolygonMesh>(np));
  }
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
  if (!np.contains("edge_is_constrained_map"))
    pm.remove_property_map(eicm);
...
auto keep_large_connected_components(PolygonMesh& pmesh,
  ...
  using Pm = PolygonMesh;
  auto eicm = get_edge_prop_map<Pm, bool>(
      pmesh, "INTERNAL_MAP0", np.contains("edge_is_constrained_map") ? np["edge_internal_map"] : py::none());
  auto fsm = get_face_prop_map<Pm, ThresholdValueType>(
      pmesh, "INTERNAL_MAP1", np.contains("face_size_map") ? np["face_size_map"] : py::none(), 1);
  bool vimap = np.contains("vertex_index_map");
  bool fimap = np.contains("face_index_map");
  std::size_t retv;
  ...
  if (vimap && fimap) {
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
        pmesh, "INTERNAL_MAP2", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    ...
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
        pmesh, "INTERNAL_MAP2", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    retv = PMP::keep_large_connected_components(
        pmesh, threshold_value,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_size_map(fsm).vertex_index_map(vim));
  } else if (fimap) {
    auto fim = get_face_prop_map<Pm, std::size_t>(pmesh, "INTERNAL_MAP2",
                                                  np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    retv = PMP::keep_large_connected_components(
        pmesh, threshold_value,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_size_map(fsm).face_index_map(fim));
  } else {
    retv = PMP::keep_large_connected_components(
        pmesh, threshold_value,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_size_map(fsm));
  }

#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
  if (!np.contains("edge_is_constrained_map"))
    pmesh.remove_property_map(eicm);
  if (!np.contains("face_size_map"))
    pmesh.remove_property_map(fsm);
#endif  // CGALPY_PMP_POLYGONAL_MESH == 1
...
auto keep_largest_connected_components(PolygonMesh& pmesh,
  ...
  if (vimap && fimap) {
    ...
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
        pmesh, "INTERNAL_MAP2", np.contains("vertex_index_map") ? np["vertex_internal_map"] : py::none());
    retv = PMP::keep_largest_connected_components(
        pmesh, nb_components_to_keep,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_size_map(fsm).vertex_index_map(vim));
  } else if (fimap) {
    auto fim = get_face_prop_map<Pm, std::size_t>(pmesh, "INTERNAL_MAP2",
                                                  np.contains("face_index_map") ? np["face_internal_map"] : py::none());
...
auto remove_connected_components_map(PolygonMesh& pm,
  ...
  if (np.contains("vertex_index_map")) {
    ...
    return PMP::remove_connected_components(pm, components_to_remove, fccmap, internal::parse_pmp_np<PolygonMesh>(np));
  }
}

template <typename PolygonMesh>
auto remove_connected_components(
    PolygonMesh& pm,
  ...
  if (vimap && fimap) {
    ...
    auto fim = get_face_prop_map<Pm, std::size_t>(pm, "INTERNAL_MAP2",
                                                  np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    PMP::remove_connected_components(
        pm, components_to_remove,
        internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).vertex_index_map(vim).face_index_map(
            fim));
  } else if (vimap) {
    auto vim = get_vertex_prop_map<Pm, std::size_t>(
    ...
    PMP::remove_connected_components(pm, components_to_remove, internal::parse_pmp_np<PolygonMesh>(np));
  }
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
  if (!np.contains("edge_is_constrained_map"))
    pm.remove_property_map(eicm);
...
auto split_connected_components(PolygonMesh& pmesh,
  ...
  // TODO: add index maps
  PMP::split_connected_components(
      pmesh, cc_meshes, internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).face_patch_map(fpm));
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
  if (!np.contains("edge_is_constrained_map"))
    pmesh.remove_property_map(eicm);
  ...
  if (np.contains("visitor")) {
    ...
    try {
      ...
      PMP::triangulate_and_refine_hole(
          pmesh, border_halfedge,
          internal::parse_pmp_np<PolygonMesh>(np).visitor(visitor).output_iterator(std::back_inserter(faces)));
    } catch (const py::cast_error&) {
      throw std::runtime_error("Visitor type not recognized");
    }
  } else {
    PMP::triangulate_and_refine_hole(
        pmesh, border_halfedge, internal::parse_pmp_np<PolygonMesh>(np).output_iterator(std::back_inserter(faces)));
  }
  return faces;
}
...
auto triangulate_hole(PolygonMesh& pmesh,
  ...
  if (np.contains("visitor")) {
    ...
    try {
      auto visitor = py::cast<pmp::HFDefault_visitor>(np["visitor"]);
      PMP::triangulate_hole(pmesh, border_halfedge, internal::parse_pmp_np<PolygonMesh>(np).visitor(visitor));
    } catch (const py::cast_error&) {
      throw std::runtime_error("Visitor type not recognized");
    }
  } else {
...
void merge_reversible_connected_components(PolygonMesh& pm, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(pm, np);
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<PolygonMesh, std::size_t>(
        pm, "INTERNAL_MAP0", np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    PMP::merge_reversible_connected_components(pm, internal::parse_pmp_np<PolygonMesh>(np).face_index_map(fim)
                                               // .vertex_point_map(vpm)
    );
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
    if (!np.contains("face_index_map"))
      pm.remove_property_map(fim);
#endif  // CGALPY_PMP_POLYGONAL_MESH == 1
  } else {
...
auto orient(TriangleMesh& tm, const py::dict& np = py::dict()) {
  ...
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<TriangleMesh, std::size_t>(
        tm, "INTERNAL_MAP0", np.contains("face_index_map") ? np["face_internal_map"] : py::none());
    PMP::orient(tm, internal::parse_pmp_np<TriangleMesh>(np).face_index_map(fim)
                // .vertex_point_map(vpm)
    );
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
    if (!np.contains("face_index_map"))
...
auto orient_polygon_soup(std::vector<Point_3>& points,
                         std::vector<std::vector<std::size_t>>& polygons,
                         const py::dict& np = py::dict()) {
  auto visitor =
      np.contains("visitor") ? py::cast<Default_orientation_visitor>(np["visitor"]) : Default_orientation_visitor();
...
auto refine_mesh_at_isolevel(PolygonMesh& pm,
                             ...
                             const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(pm, np);
  auto eicm = get_edge_prop_map<PolygonMesh, bool>(
      pm, "INTERNAL_MAP0", np.contains("edge_is_constrained_map") ? np["edge_internal_map"] : py::none());
  return PMP::refine_mesh_at_isolevel(
      pm, value_map, isovalue, internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm)
      // .vertex_point_map(vpm)
  );
}
...
template <typename PolygonMesh>
py::tuple corefine_and_compute_boolean_operations(PolygonMesh& pm1,
                                                  PolygonMesh& pm2,
                                                  const py::dict& np1 = py::dict(),
                                                  const py::dict& np2 = py::dict(),
  ...
  // auto vpm1 = get_vertex_point_map(pm1, np1);
  // auto vpm2 = get_vertex_point_map(pm2, np2);
  auto eicm1 = get_edge_prop_map<Pm, bool>(
      pm1, "INTERNAL_MAP0", np1.contains("edge_is_constrained_map") ? np1["edge_internal_map"] : py::none());
  auto eicm2 = get_edge_prop_map<Pm, bool>(
      pm2, "INTERNAL_MAP1", np2.contains("edge_is_constrained_map") ? np2["edge_internal_map"] : py::none());
  ...
  if (fimb1 && fimb2) {
    if (visitor_flag) {
      try {
        ...
        auto fim1 = get_face_prop_map<Pm, std::size_t>(
            pm1, "INTERNAL_MAP6", np1.contains("face_index_map") ? np1["face_internal_map"] : py::none());
        auto fim2 = get_face_prop_map<Pm, std::size_t>(
            pm2, "INTERNAL_MAP7", np2.contains("face_index_map") ? np2["face_internal_map"] : py::none());
        res = PMP::corefine_and_compute_boolean_operations(
            ...
            internal::parse_pmp_np<PolygonMesh>(np2)
                ...
                // .vertex_point_map(vpm2)
                .edge_is_constrained_map(eicm2),
            std::make_tuple(internal::parse_pmp_np<PolygonMesh>(np_out_arr[0])
    ...
    if (visitor_flag) {
      try {
        auto visit = py::cast<pmp::Corefine_visitor<Pm>>(np1["visitor"]);
        auto fim2 = get_face_prop_map<Pm, std::size_t>(
            pm2, "INTERNAL_MAP6", np2.contains("face_index_map") ? np2["face_internal_map"] : py::none());
        res = PMP::corefine_and_compute_boolean_operations(
            pm1, pm2, outputs,
            internal::parse_pmp_np<PolygonMesh>(np1)
                // .vertex_point_map(vpm1)
    ...
    res =
        PMP::corefine_and_compute_boolean_operations(pm1, pm2, outputs,
                                                     internal::parse_pmp_np<PolygonMesh>(np1)
                                                         // .vertex_point_map(vpm1)
                                                         .edge_is_constrained_map(eicm1),
                                                     internal::parse_pmp_np<PolygonMesh>(np2)
  ...
  auto retv =
      py::make_tuple(res[0] ? py::cast(out_union) : py::none(), res[1] ? py::cast(out_intersection) : py::none(),
                     res[2] ? py::cast(tm1_minus_tm2) : py::none(), res[3] ? py::cast(tm2_minus_tm1) : py::none());
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
  if (!np1.contains("edge_is_constrained_map"))
    pm1.remove_property_map(eicm1);
  if (!np2.contains("edge_is_constrained_map"))
    pm2.remove_property_map(eicm2);
  if (!np_out_arr[0].contains("edge_is_constrained_map"))
    out_union.remove_property_map(eicm_out1);
  if (!np_out_arr[1].contains("edge_is_constrained_map"))
    out_intersection.remove_property_map(eicm_out2);
  if (!np_out_arr[2].contains("edge_is_constrained_map"))
    tm1_minus_tm2.remove_property_map(eicm_out3);
  if (!np_out_arr[3].contains("edge_is_constrained_map"))
    tm2_minus_tm1.remove_property_map(eicm_out4);
#endif  // CGALPY_PMP_POLYGONAL_MESH == 1
  return retv;
}
...
//
template <typename TriangleMesh>
TriangleMesh corefine_and_compute_intersection(TriangleMesh& pm1,
                                               TriangleMesh& pm2,
                                               const py::dict& np1 = py::dict(),
                                               const py::dict& np2 = py::dict(),
  ...
  auto eicm1 = get_edge_prop_map<Tm, bool>(
      pm1, "INTERNAL_MAP0", np1.contains("edge_is_constrained_map") ? np1["edge_internal_map"] : py::none());
  auto eicm2 = get_edge_prop_map<Tm, bool>(
      pm2, "INTERNAL_MAP1", np2.contains("edge_is_constrained_map") ? np2["edge_internal_map"] : py::none());
  auto eicm_out = get_edge_prop_map<Tm, bool>(
      out, "INTERNAL_MAP2", np_out.contains("edge_is_constrained_map") ? np_out["edge_internal_map"] : py::none());
  ...
  if (fimb1 && fimb2) {
    if (visitor_flag) {
      try {
        ...
        auto fim1 = get_face_prop_map<Tm, std::size_t>(
            pm1, "INTERNAL_MAP3", np1.contains("face_index_map") ? np1["face_internal_map"] : py::none());
        auto fim2 = get_face_prop_map<Tm, std::size_t>(
            pm2, "INTERNAL_MAP4", np2.contains("face_index_map") ? np2["face_internal_map"] : py::none());
    ...
    if (visitor_flag) {
      try {
        auto visitor = py::cast<pmp::Corefine_visitor<Tm>>(np1["visitor"]);
        auto fim2 = get_face_prop_map<Tm, std::size_t>(
            pm2, "INTERNAL_MAP3", np2.contains("face_index_map") ? np2["face_internal_map"] : py::none());
        valid = PMP::corefine_and_compute_intersection(pm1, pm2, out,
                                                       internal::parse_pmp_np<TriangleMesh>(np1)
                                                           // .vertex_point_map(vpm1)
                                                           .edge_is_constrained_map(eicm1)
  ...
  if (!np2.contains("edge_is_constrained_map"))
    pm2.remove_property_map(eicm2);
  if (!np_out.contains("edge_is_constrained_map"))
    out.remove_property_map(eicm_out);
#endif  // CGALPY_PMP_POLYGONAL_MESH == 1
  if (!valid)
    throw std::runtime_error("Cannot compute difference!");
  return out;
}

//
template <typename TriangleMesh>
TriangleMesh corefine_and_compute_difference(TriangleMesh& pm1,
                                             TriangleMesh& pm2,
                                             const py::dict& np1 = py::dict(),
                                             const py::dict& np2 = py::dict(),
                                             const py::dict& np_out = py::dict()) {
...
//
template <typename TriangleMesh>
TriangleMesh corefine_and_compute_union(TriangleMesh& pm1,
                                        TriangleMesh& pm2,
                                        const py::dict& np1 = py::dict(),
                                        const py::dict& np2 = py::dict(),
                                        const py::dict& np_out = py::dict()) {
...
template <typename PolygonMesh>
auto triangulate_hole_polyline(const std::vector<Point_3>& polyline, const py::dict& np = py::dict()) {
  using Triangle_int = CGAL::Triple<int, int, int>;
  std::vector<Triangle_int> out;
  if (np.contains("visitor")) {
    // HFDefault_visitor
    ...
    PMP::triangulate_hole_polyline(polyline, std::back_inserter(out), internal::parse_pmp_np<PolygonMesh>(np));
  }
  std::array<int, 3> out2 = {out[0].first, out[0].second, out[0].third};
  return out2;
}
...
auto triangulate_refine_and_fair_hole(PolygonMesh& pmesh,
  ...
  if (np.contains("visitor")) {
    ...
    auto res = PMP::triangulate_refine_and_fair_hole(
        pmesh, border_halfedge,
        internal::parse_pmp_np<PolygonMesh>(np).face_output_iterator(it1).vertex_output_iterator(it2).visitor(visitor));
    return py::make_tuple(std::get<0>(res), fids, vids);
  } else {
    auto res = PMP::triangulate_refine_and_fair_hole(
        pmesh, border_halfedge,
        internal::parse_pmp_np<PolygonMesh>(np).face_output_iterator(it1).vertex_output_iterator(it2));
    return py::make_tuple(std::get<0>(res), fids, vids);
  }
}
...
auto triangulate_faces_r(const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& face_range,
  ...
  using Fd = typename Gt::face_descriptor;
  // auto vpm = get_vertex_point_map(pm, np);
  return PMP::triangulate_faces(face_range, pm, internal::parse_pmp_np<PolygonMesh>(np)
                                // .vertex_point_map(vpm)
  );
}
...
auto isotropic_remeshing_sf(const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& face_range,
  ...
  PMP::isotropic_remeshing(face_range, sizing, pmesh, internal::parse_pmp_np<PolygonMesh>(np));
}
...
auto remesh_planar_patches(PolygonMesh& pmesh,
  ...
  auto vcm = get_vertex_prop_map<Pm, std::size_t>(
      pmesh, "vertex_corner_map", np_in.contains("vertex_corner_map") ? np_in["vertex_corner_map"] : py::none());
  ...
  auto vcm2 = get_vertex_prop_map<Pm, std::size_t>(
      pmesh, "vertex_corner_map", np_out.contains("vertex_corner_map") ? np_out["vertex_corner_map"] : py::none());

  Pm out;

  PMP::remesh_planar_patches(
      pmesh, out,
      internal::parse_pmp_np<PolygonMesh>(np_in).edge_is_constrained_map(eicm).face_patch_map(fpm).vertex_corner_map(
          vcm),
      internal::parse_pmp_np<PolygonMesh>(np_out).face_patch_map(fpm2).vertex_corner_map(vcm2));
#if CGALPY_PMP_POLYGONAL_MESH == 1  // surface_mesh
...
auto smooth_shape(PolygonMesh& pmesh, const double time, const py::dict& np = py::dict()) {
  ...
  return PMP::smooth_shape(
      pmesh, time, internal::parse_named_parameters(np).vertex_is_constrained_map(propmap).vertex_point_map(vpm));
}
...
auto approximate_max_distance_to_point_set(const TriangleMesh& tm,
                                           ...
                                           const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tm, np);
  return PMP::approximate_max_distance_to_point_set(tm, points, precision, internal::parse_pmp_np<TriangleMesh>(np)
                                                    // .vertex_point_map(vpm)
  );
}
...
template <typename PolygonMesh>
auto is_Hausdorff_distance_larger(const PolygonMesh& tm1,
                                  const PolygonMesh& tm2,
...
auto max_distance_to_triangle_mesh(const std::vector<Point_3>& points,
  ...
  // auto vpm = get_vertex_point_map(tm, np);
  using TAG = CGAL::Sequential_tag;
  return PMP::max_distance_to_triangle_mesh<TAG>(points, tm, internal::parse_pmp_np<TriangeMesh>(np)
                                                 // .vertex_point_map(vpm)
  );
}
...
bool clip(PolygonMesh& tm,
  ...
  auto eicm1 = get_edge_prop_map<Pm, bool>(
      tm, "INTERNAL_MAP0", np_tm.contains("edge_is_constrained_map") ? np_tm["edge_internal_map"] : py::none());
  auto eicm2 = get_edge_prop_map<Pm, bool>(
      clipper, "INTERNAL_MAP1", np_c.contains("edge_is_constrained_map") ? np_c["edge_internal_map"] : py::none());
  ...
  if (visitor1 && visitor2) {
    try {
      ...
      res =
          PMP::clip(tm, clipper, internal::parse_pmp_np<PolygonMesh>(np_tm).visitor(v1).edge_is_constrained_map(eicm1),
...
auto stitch_borders_bc(
  ...
  if (np.contains("face_index_map")) {
    ...
    return PMP::stitch_borders(boundary_cycle_representatives, pmesh, internal::parse_pmp_np<PolygonMesh>(np)
                               // .vertex_point_map(vpm)
    );
  }
}
...
auto stitch_borders_he(
  ...
  using Hd = typename Gt::halfedge_descriptor;
  // auto vpm = get_vertex_point_map(pmesh, np);
  return PMP::stitch_borders(pmesh, hedge_pairs_to_stitch, internal::parse_pmp_np<PolygonMesh>(np)
                             // .vertex_point_map(vpm)
  );
}

template <typename PolygonMesh>
auto stitch_borders(PolygonMesh& pmesh, const py::dict& np = py::dict()) {
  // return PMP::stitch_borders(pmesh, internal::parse_pmp_np<PolygonMesh>(np));
  // auto vpm = get_vertex_point_map(pmesh, np);
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<PolygonMesh, std::size_t>(
        pmesh, "INTERNAL_MAP0", np.contains("face_index_map") ? np["face_index_map"] : py::none());
    return PMP::stitch_borders(pmesh, internal::parse_pmp_np<PolygonMesh>(np)
                                          // .vertex_point_map(vpm)
                                          .face_index_map(fim));
  } else {
...
auto stitch_boundary_cycles(
  ...
  using Hd = typename Gt::halfedge_descriptor;
  // auto vpm = get_vertex_point_map(pmesh, np);
  return PMP::stitch_boundary_cycles(boundary_cycle_representatives, pmesh, internal::parse_pmp_np<PolygonMesh>(np)
                                     // .vertex_point_map(vpm)
  );
}

template <typename PolygonMesh>
auto remove_almost_degenerate_faces_r(
    const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& face_range,
    PolygonMesh& pmesh,
    const py::dict& np = py::dict()) {
  using Pm = PolygonMesh;
  ...
  // auto vpm = get_vertex_point_map(pmesh, np);
  auto eicm = get_edge_prop_map<Pm, bool>(
      pmesh, "INTERNAL_MAP0", np.contains("edge_is_constrained_map") ? np["edge_is_constrained_map"] : py::none());
  auto vicm = get_vertex_prop_map<Pm, bool>(
      pmesh, "INTERNAL_MAP1", np.contains("vertex_is_constrained_map") ? np["vertex_is_constrained_map"] : py::none());
...
auto remove_almost_degenerate_faces(TriangleMesh& tmesh, const py::dict& np = py::dict()) {
  ...
  auto eicm = get_edge_prop_map<TriangleMesh, bool>(
      tmesh, "INTERNAL_MAP0", np.contains("edge_is_constrained_map") ? np["edge_is_constrained_map"] : py::none());
  auto vicm = get_vertex_prop_map<TriangleMesh, bool>(
      tmesh, "INTERNAL_MAP1", np.contains("vertex_is_constrained_map") ? np["vertex_is_constrained_map"] : py::none());
  std::function<bool(Point_3, Point_3, Point_3)> filter = [](const Point_3&, const Point_3&, const Point_3&) {
    return true;
  };
  if (np.contains("filter")) {
  ...
  Filter f;
  f.filter = filter;
  auto retv = PMP::remove_almost_degenerate_faces(tmesh, internal::parse_pmp_np<TriangleMesh>(np)
                                                             // .vertex_point_map(vpm)
                                                             .edge_is_constrained_map(eicm)
                                                             .vertex_is_constrained_map(vicm)
                                                             .filter(f));
...
template <typename TriangleMesh>
auto remove_connected_components_of_negligible_size(TriangleMesh& tmesh, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tmesh, np);
  auto eicm = get_edge_prop_map<TriangleMesh, bool>(
      tmesh, "INTERNAL_MAP0", np.contains("edge_is_constrained_map") ? np["edge_is_constrained_map"] : py::none());
  ...
  std::size_t retv;
  if (fim_flag) {
    auto fim = get_face_prop_map<TriangleMesh, std::size_t>(tmesh, "INTERNAL_MAP1", np["face_index_map"]);
    retv = PMP::remove_connected_components_of_negligible_size(tmesh, internal::parse_pmp_np<TriangleMesh>(np)
                                                                          // .vertex_point_map(vpm)
                                                                          .edge_is_constrained_map(eicm)
                                                                          .face_index_map(fim));
  } else {
...
auto random_perturbation(TriangleMesh& tmesh, const double& perturbation_max_size, const py::dict& np = py::dict()) {
  ...
  PMP::random_perturbation(tmesh, perturbation_max_size,
                           internal::parse_pmp_np<TriangleMesh>(np).vertex_point_map(vicm));
...
auto random_perturbation_v(const std::vector<typename boost::graph_traits<PolygonMesh>::vertex_descriptor>& vertices,
  ...
  PMP::random_perturbation(vertices, pmesh, perturbation_max_size,
                           internal::parse_pmp_np<PolygonMesh>(np).vertex_point_map(vicm));
}
...
auto compute_face_normals(const PolygonMesh& pmesh, Face_normal_map face_normals, const py::dict& np = py::dict()) {
  using Pm = PolygonMesh;
  // auto vpm = get_vertex_point_map(pmesh, np);
  return PMP::compute_face_normals(pmesh, face_normals, internal::parse_pmp_np<Pm>(np).face_normal_map(face_normals)
                                   // .vertex_point_map(vpm)
  );
}
...
auto compute_vertex_normals(const PolygonMesh& pmesh, VertexNormalMap vertex_normals, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(sm, np);
...
auto area_f(const std::vector<typename boost::graph_traits<TriangleMesh>::face_descriptor>& face_range,
  ...
  using Fd = typename Gt::face_descriptor;
  // auto vpm = get_vertex_point_map(tm, np);
  return PMP::area(face_range, tm, internal::parse_pmp_np<TriangleMesh>(np)
                   // .vertex_point_map(vpm)
  );
}
...
auto longest_border(const TriangleMesh& tm, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tm, np);
  return PMP::longest_border(tm, internal::parse_pmp_np<TriangleMesh>(np)
                             // .vertex_point_map(vpm)
  );
}
...
template <typename TriangleMesh>
auto does_bound_a_volume(TriangleMesh& tm, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tm, np);
  bool retv;
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<TriangleMesh, bool>(tm, "INTERNAL_MAP0",
                                                     np.contains("face_index_map") ? np["face_index_map"] : py::none());
    retv = PMP::does_bound_a_volume(tm, internal::parse_pmp_np<TriangleMesh>(np).face_index_map(fim)
                                    // .vertex_point_map(vpm)
    );
  } else {
...
void interpolated_corrected_curvatures(PolygonMesh& pmesh, const py::dict& np = py::dict()) {
  ...
  auto vgcm = get_vertex_prop_map<PolygonMesh, FT>(
      pmesh, "INTERNAL_MAP1",
      np.contains("vertex_Gaussian_curvature_map") ? np["vertex_Gaussian_curvature_map"] : py::none());
  auto vpcdm = get_vertex_prop_map<PolygonMesh, PMP::Principal_curvatures_and_directions<Kernel>>(
      ...
      np.contains("vertex_principal_curvatures_and_directions_map")
          ? np["vertex_principal_curvatures_and_directions_map"]
          : py::none());
...
auto border_halfedges(const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& face_range,
  ...
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<Pm, std::size_t>(
        pmesh, "INTERNAL_MAP0", np.contains("face_index_map") ? np["face_patch_index_map"] : py::none());
    PMP::border_halfedges(face_range, pmesh, std::back_inserter(out),
                          internal::parse_pmp_np<PolygonMesh>(np).face_index_map(fim));
  } else {
    PMP::border_halfedges(face_range, pmesh, std::back_inserter(out), internal::parse_pmp_np<PolygonMesh>(np));
  }
  return out;
}
  ...
  if (visitor1 && visitor2) {
    try {
      ...
      PMP::split(pm, splitter,
                 internal::parse_pmp_np<PolygonMesh>(np_tm).visitor(v1)
                 // .vertex_point_map(vpm1)
                 ,
    ...
    try {
      auto v2 = py::cast<pmp::Corefine_visitor<PolygonMesh>>(np_s["visitor"]);
      PMP::split(pm, splitter,
                 internal::parse_pmp_np<PolygonMesh>(np_tm)
                 // .vertex_point_map(vpm1)
                 ,
                 internal::parse_pmp_np<PolygonMesh>(np_s).visitor(v2)
                 // .vertex_point_map(vpm2)
...
template <typename TriangleMesh>
auto split_p(TriangleMesh& tm, const Plane_3& plane, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tm, np);
...
auto merge_duplicate_points_in_polygon_soup(std::vector<Point_3>& pointvec,
                                            ...
                                            const py::dict& np = py::dict()) {
  return PMP::merge_duplicate_points_in_polygon_soup(pointvec, polyvec, internal::parse_named_parameters(np));
}

auto merge_duplicate_polygons_in_polygon_soup(std::vector<Point_3>& points,
                                              std::vector<std::vector<std::size_t>>& polygons,
                                              const py::dict& np = py::dict()) {
  return PMP::merge_duplicate_polygons_in_polygon_soup(points, polygons, internal::parse_named_parameters(np));
}
...
auto polygon_mesh_to_polygon_soup(const PolygonMesh& pm, const py::dict& np = py::dict()) {
  ...
  PMP::polygon_mesh_to_polygon_soup(pm, pts, polys, internal::parse_pmp_np<Pm>(np));
  return std::make_tuple(pts, polys);
}

template <typename PolygonMesh>
auto orient_triangle_soup_with_reference_triangle_mesh(const PolygonMesh& tm_ref,
...
auto orient_triangle_soup_with_reference_triangle_soup(const std::vector<Point_3>& ref_points,
                                                       ...
                                                       const py::dict& np1 = py::dict(),
                                                       const py::dict& np2 = py::dict()) {
  PMP::orient_triangle_soup_with_reference_triangle_soup(ref_points, ref_faces, points,
                                                         faces);  // doesn't work for some reason
}
...
auto duplicate_non_manifold_vertices(PolygonMesh& pm, const py::dict& np = py::dict()) {
  ...
  auto nb = PMP::duplicate_non_manifold_vertices(pm, internal::parse_pmp_np<PolygonMesh>(np)
                                                         // .vertex_point_map(vpm)
...
void detect_sharp_edges(PolygonMesh& pmesh, double angle_in_deg, EBMap ebmap, const py::dict& np = py::dict()) {
  if (np.contains("vertex_face_degree_map")) {
    ...
    PMP::detect_sharp_edges(pmesh, angle_in_deg, ebmap,
                            internal::parse_pmp_np<PolygonMesh>(np).vertex_feature_degree_map(vfdm));
  } else {
...
auto region_growing_of_planes_on_faces(PolygonMesh& pmesh, RegionMap region_map, const py::dict& np = py::dict()) {
  ...
  if (np.contains("region_primitive_map")) {
    ...
    num_regions = PMP::region_growing_of_planes_on_faces(
        pmesh, region_map, internal::parse_pmp_np<PolygonMesh>(np).region_primitive_map(rpm));
  } else {
    num_regions = PMP::region_growing_of_planes_on_faces(pmesh, region_map, internal::parse_pmp_np<PolygonMesh>(np));
  }
  return num_regions;
}
...
auto detect_corners_of_regions(PolygonMesh& pmesh,
  ...
  std::size_t r = PMP::detect_corners_of_regions(pmesh, region_map, nb_regions, corner_id_map,
                                                 internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm)
                                                 // .vertex_point_map(vpm)
  );
#if CGALPY_PMP_POLYGONAL_MESH == 1
  if (!np.contains("edge_is_constrained_map")) {
...
auto remesh_almost_planar_patches(PolygonMesh tm_in,
  ...
  auto vcm = get_vertex_prop_map<Pm, int>(
      pm_out, "INTERNAL_MAP2", np_out.contains("vertex_corner_map") ? np_out["vertex_corner_map"] : py::none());
  ...
  if (np_in.contains("patch_normal_map")) {
    ...
    retv = PMP::remesh_almost_planar_patches(
        tm_in, pm_out, nb_patches, nb_corners, face_patch_map, vertex_corner_map, ecm,
        internal::parse_pmp_np<PolygonMesh>(np_in).patch_normal_map(rpm),
        internal::parse_pmp_np<PolygonMesh>(np_out).face_patch_map(fpm).vertex_corner_map(vcm));
  } else {
    retv = PMP::remesh_almost_planar_patches(
        tm_in, pm_out, nb_patches, nb_corners, face_patch_map, vertex_corner_map, ecm,
...
auto angle_and_area_smoothing(const std::vector<typename boost::graph_traits<PolygonMesh>::face_descriptor>& faces,
  ...
  PMP::angle_and_area_smoothing(
      faces, pmesh,
      internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).vertex_is_constrained_map(vicm));
#if CGALPY_PMP_POLYGONAL_MESH == 1
  if (!np.contains("edge_is_constrained_map")) {
    pmesh.remove_property_map(eicm);
  }
...
auto compatible_orientations(PolygonMesh& pm, FaceBitMap fbm, const py::dict& np = py::dict()) {
  using Pm = PolygonMesh;
  auto fpim = get_face_prop_map<Pm, std::size_t>(
      pm, "INTERNAL_MAP0", np.contains("face_patch_index_map") ? np["face_patch_index_map"] : py::none());

  auto retv =
      PMP::compatible_orientations(pm, fbm, internal::parse_pmp_np<PolygonMesh>(np).face_partition_id_map(fpim));
#if CGALPY_PMP_POLYGONAL_MESH == 1
...
auto surface_Delaunay_remeshing(PolygonMesh& tmesh, const py::dict& np = py::dict()) {
  ...
  if (np.contains("mesh_edge_size")) {
    ...
    retv = PMP::surface_Delaunay_remeshing(tmesh, internal::parse_pmp_np<PolygonMesh>(np)
                                                      .edge_is_constrained_map(eicm)
                                                      .face_patch_map(fpm)
                                                      .facet_size(mfs)
                                                      .facet_angle(mfa)
...
typename boost::graph_traits<PolygonMesh>::faces_size_type sharp_edges_segmentation(
  ...
  if (fimap && vimap) {
    auto fim = get_face_prop_map<Pm, std::size_t>(pmesh, "INTERNAL_MAP1",
                                                  np.contains("face_index_map") ? np["face_index_map"] : py::none());
    auto vipm = get_vertex_prop_map<Pm, std::set<int>>(
        pmesh, "INTERNAL_MAP2", np.contains("vertex_index_map") ? np["vertex_index_map"] : py::none());
    ...
    auto fim = get_face_prop_map<Pm, std::size_t>(pmesh, "INTERNAL_MAP1",
                                                  np.contains("face_index_map") ? np["face_index_map"] : py::none());
    num_patches = PMP::sharp_edges_segmentation(
        pmesh, angle_in_deg, edge_is_feature_map, patch_id_map,
        internal::parse_pmp_np<PolygonMesh>(np).vertex_feature_degree_map(vfdm).face_index_map(fim));
  } else if (vimap) {
    auto vipm = get_vertex_prop_map<Pm, std::set<int>>(
        pmesh, "INTERNAL_MAP2", np.contains("vertex_index_map") ? np["vertex_index_map"] : py::none());
    num_patches = PMP::sharp_edges_segmentation(
        pmesh, angle_in_deg, edge_is_feature_map, patch_id_map,
        internal::parse_pmp_np<PolygonMesh>(np).vertex_feature_degree_map(vfdm).vertex_incident_patches_map(vipm));
  } else {
    num_patches =
        PMP::sharp_edges_segmentation(pmesh, angle_in_deg, edge_is_feature_map, patch_id_map,
                                      internal::parse_pmp_np<PolygonMesh>(np).vertex_feature_degree_map(vfdm));
  }
#if CGALPY_PMP_POLYGONAL_MESH == 1
  if (!np.contains("vertex_face_degree_map")) {
...
template <typename TriangleMesh>
auto get_descriptor_from_location(
    const std::pair<typename boost::graph_traits<TriangleMesh>::face_descriptor, std::array<FT, 3>>& loc,
    const TriangleMesh& tm) {
  using Tm = TriangleMesh;
  using Gt = boost::graph_traits<Tm>;
  using Fd = typename Gt::face_descriptor;
...
template <typename TriangleMesh>
auto is_on_vertex(const std::pair<typename boost::graph_traits<TriangleMesh>::face_descriptor, std::array<FT, 3>>& loc,
...
auto angle_and_area_smoothing_m(PolygonMesh& pmesh, const py::dict& np = py::dict()) {
  ...
  auto vicm = get_vertex_prop_map<Pm, bool>(
      pmesh, "INTERNAL_MAP1", np.contains("vertex_is_constrained_map") ? np["vertex_is_constrained_map"] : py::none());
  PMP::angle_and_area_smoothing(
      pmesh, internal::parse_pmp_np<PolygonMesh>(np).edge_is_constrained_map(eicm).vertex_is_constrained_map(vicm));
#if CGALPY_PMP_POLYGONAL_MESH == 1
  if (!np.contains("edge_is_constrained_map")) {
    pmesh.remove_property_map(eicm);
  }
...
auto extrude_mesh_v(const InputMesh& imesh,
                    ...
                    const py::dict& np_in = py::dict(),
                    const py::dict& np_out = py::dict()) {
  PMP::extrude_mesh(imesh, omesh, v, internal::parse_pmp_np<OutputMesh>(np_in),
...
auto volume_connected_components(TriangleMesh& tm, VolumeFaceIndexMap volume_id_map, const py::dict& np = py::dict()) {
  ...
  if (np.contains("face_index_map")) {
    ...
    if (np.contains("face_connected_component_map")) {
      auto fccm = get_face_prop_map<TriangleMesh, std::size_t>(
          tm, "INTERNAL_MAP1",
          np.contains("face_connected_component_map") ? np["face_connected_component_map"] : py::none());
      retv =
          PMP::volume_connected_components(tm, volume_id_map,
...
template <typename PolygonMesh>
void orient_to_bound_a_volume(PolygonMesh& tm, const py::dict& np = py::dict()) {
  // auto vpm = get_vertex_point_map(tm, np);
  if (np.contains("face_index_map")) {
    auto fim = get_face_prop_map<PolygonMesh, std::size_t>(
        tm, "INTERNAL_MAP0", np.contains("face_index_map") ? np["face_index_map"] : py::none());
...
// visitor stuff
void set_polygon_orientation_reversed(Default_orientation_visitor& v, const std::function<void(std::size_t)>& f) {
...
void set_duplicated_vertex(Default_orientation_visitor& v, const std::function<void(std::size_t, std::size_t)>& f) {
...
void set_non_manifold_vertex(Default_orientation_visitor& v, const std::function<void(std::size_t, std::size_t)>& f) {
...
void set_before_subface_creations_fn(Cv& v, const std::function<void(Fd, Pm&)>& f) {
  v.set_before_subface_creations(f);
}
void set_after_subface_creations_fn(Cv& v, const std::function<void(Pm&)>& f) {
  v.set_after_subface_creations(f);
}
void set_before_subface_created_fn(Cv& v, const std::function<void(Pm&)>& f) {
  v.set_before_subface_created(f);
}
void set_after_subface_created_fn(Cv& v, const std::function<void(Fd, Pm&)>& f) {
  v.set_after_subface_created(f);
}
void set_before_face_copy_fn(Cv& v, const std::function<void(Fd, const Pm&, Pm&)>& f) {
  v.set_before_face_copy(f);
}
void set_after_face_copy_fn(Cv& v, const std::function<void(Fd, const Pm&, Fd, Pm&)>& f) {
  v.set_after_face_copy(f);
}
void set_before_edge_split_fn(Cv& v, const std::function<void(Hd, Pm&)>& f) {
  v.set_before_edge_split(f);
}
void set_edge_split_fn(Cv& v, const std::function<void(Hd, Pm&)>& f) {
  v.set_edge_split(f);
}
void set_after_edge_split_fn(Cv& v, const std::function<void()>& f) {
  v.set_after_edge_split(f);
}
void set_add_retriangulation_edge_fn(Cv& v, const std::function<void(Hd, Pm&)>& f) {
  v.set_add_retriangulation_edge(f);
}
void set_before_edge_copy_fn(Cv& v, const std::function<void(Hd, const Pm&, Pm&)>& f) {
  v.set_before_edge_copy(f);
}
void set_after_edge_copy_fn(Cv& v, const std::function<void(Hd, const Pm&, Hd, Pm&)>& f) {
  v.set_after_edge_copy(f);
}
void set_before_edge_duplicated_fn(Cv& v, const std::function<void(Hd, Pm&)>& f) {
  v.set_before_edge_duplicated(f);
}
void set_after_edge_duplicated_fn(Cv& v, const std::function<void(Hd, Hd, Pm&)>& f) {
...
void set_intersection_point_detected_fn(
    Cv& v,
    const std::function<void(std::size_t, int, Hd, Hd, const Pm&, const Pm&, bool, bool)>& f) {
  v.set_intersection_point_detected(f);
}
void set_before_vertex_copy_fn(Cv& v, const std::function<void(Vd, const Pm&, Pm&)>& f) {
...
void set_triangulating_faces_step_fn(Cv& v, const std::function<void()>& f) {
  v.set_triangulating_faces_step(f);
}
void set_end_triangulating_faces_fn(Cv& v, const std::function<void()>& f) {
...
void set_start_building_output_fn(Cv& v, const std::function<void()>& f) {
  v.set_start_building_output(f);
}
void set_end_building_output_fn(Cv& v, const std::function<void()>& f) {
  v.set_end_building_output(f);
}
void set_filter_coplanar_edges_fn(Cv& v, const std::function<void()>& f) {
  v.set_filter_coplanar_edges(f);
}
void set_detect_patches_fn(Cv& v, const std::function<void()>& f) {
  v.set_detect_patches(f);
}
void set_classify_patches_fn(Cv& v, const std::function<void()>& f) {
...
void export_polygon_mesh_processing(py::module_& m) {
  ...
  // FacePatchMap	a class model of ReadablePropertyMap with boost::graph_traits<TriangleMeshIn>::face_descriptor
  // as key type and std::size_t as value type EdgeIsConstrainedMap	a class model of ReadablePropertyMap with
  // boost::graph_traits<TriangleMeshIn>::edge_descriptor as key type and bool as value type VertexCornerMap	a class
  // model of ReadablePropertyMap with boost::graph_traits<TriangleMeshIn>::vertex_descriptor as key type and
  ...
  py::class_<Np_class>(m, "Named_function_parameters").def(py::init<>());
  ...
  // m.def("connected_components", &pmp::connected_components_map<Pm, boost::property_map<Pm,
  // CGAL::dynamic_face_property_t<std::uint32_t>>::type>,
  //       py::arg("pm"), py::arg("fcm"), py::arg("np") = py::dict());
#endif
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("connected_components", &pmp::connected_components_map<Pm, Pm::Property_map<Fd, std::size_t>>, py::arg("pm"),
        py::arg("fcm"), py::arg("np") = py::dict());
  m.def("connected_components", &pmp::connected_components_map<Pm, Pm::Property_map<Fd, std::uint32_t>>, py::arg("pm"),
        py::arg("fcm"), py::arg("np") = py::dict());
  m.def("keep_connected_components", &pmp::keep_connected_components_map<Pm, FaceComponentMap>, py::arg("pm"),
        py::arg("components_to_keep"), py::arg("fcm"), py::arg("np") = py::dict());
#endif
  m.def("keep_connected_components", &pmp::keep_connected_components<Pm>, py::arg("pm"), py::arg("components_to_keep"),
        py::arg("np") = py::dict());
  m.def("keep_large_connected_components", &pmp::keep_large_connected_components<Pm, std::size_t>, py::arg("pm"),
        py::arg("min_size"), py::arg("np") = py::dict());
  m.def("keep_large_connected_components", &pmp::keep_large_connected_components<Pm, double>, py::arg("pm"),
        py::arg("min_size"), py::arg("np") = py::dict());
  m.def("keep_largest_connected_components", &pmp::keep_largest_connected_components<Pm>, py::arg("pm"),
        py::arg("nb_components_to_keep"), py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("remove_connected_components", &pmp::remove_connected_components_map<Pm, FaceComponentMap>, py::arg("pm"),
        py::arg("components_to_remove"), py::arg("fccmap"), py::arg("np") = py::dict());
#endif
  m.def("remove_connected_components", &pmp::remove_connected_components<Pm>, py::arg("pm"),
        py::arg("components_to_remove"), py::arg("np") = py::dict());
  m.def("split_connected_components", &pmp::split_connected_components<Pm>, py::arg("pmesh"), py::arg("cc_meshes"),
        py::arg("np") = py::dict());

  // Hole Filling
  m.def("triangulate_and_refine_hole", &pmp::triangulate_and_refine_hole<Pm>, py::arg("pm"), py::arg("hole_boundary"),
        py::arg("np") = py::dict());
  m.def("triangulate_hole", &pmp::triangulate_hole<Pm>, py::arg("pmesh"), py::arg("border_halfedge"),
        py::arg("np") = py::dict());
  m.def("triangulate_hole_polyline", &pmp::triangulate_hole_polyline<Pm>, py::arg("points"),
        py::arg("np") = py::dict());
  m.def("triangulate_hole_polyline", &pmp::triangulate_hole_polyline_2<Pm>, py::arg("points"), py::arg("third_points"),
        py::arg("np") = py::dict());
  m.def("triangulate_refine_and_fair_hole", &pmp::triangulate_refine_and_fair_hole<Pm>, py::arg("pmesh"),
        py::arg("border_halfedge"), py::arg("np") = py::dict());

  // Meshing
  m.def("angle_and_area_smoothing", &pmp::angle_and_area_smoothing<Pm>, py::arg("faces"), py::arg("tmesh"),
        py::arg("np") = py::dict());
  m.def("angle_and_area_smoothing", &pmp::angle_and_area_smoothing_m<Pm>, py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("extrude_mesh", &pmp::extrude_mesh_v<Pm, Pm>, py::arg("imesh"), py::arg("omesh"), py::arg("v"),
  ...
  m.def("fair", &pmp::fair<Pm>,
        ...
        py::arg("np") = py::dict());
#endif
  m.def("isotropic_remeshing", &pmp::isotropic_remeshing_sf<Pm, pmp::Adaptive_sizing_field<Pm>>, py::arg("faces"),
        py::arg("sizing"), py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("isotropic_remeshing", &pmp::isotropic_remeshing_sf<Pm, pmp::Uniform_sizing_field<Pm>>, py::arg("faces"),
        py::arg("target_edge_length"), py::arg("pmesh"), py::arg("np") = py::dict());
  ...
  //       py::arg("np") = py::dict());
  m.def("random_perturbation", &pmp::random_perturbation<Pm>, py::arg("tmesh"), py::arg("perturbation_max_size"),
        py::arg("np") = py::dict());
  m.def("random_perturbation", &pmp::random_perturbation_v<Pm>, py::arg("vertices"), py::arg("tmesh"),
        py::arg("perturbation_max_size"), py::arg("np") = py::dict());
  m.def("refine", &pmp::refine<Pm>, py::arg("tmesh"), py::arg("faces"), py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1  // only working for sm
  m.def("remesh_almost_planar_patches",
        &pmp::remesh_almost_planar_patches<Pm, FacePatchMap, VertexCornerMap, EdgeIsConstrainedMap>, py::arg("tm_in"),
        py::arg("nb_patches"), py::arg("nb_corners"), py::arg("face_patch_map"), py::arg("vertex_corner_map"),
        py::arg("ecm"), py::arg("np_in") = py::dict(), py::arg("np_out") = py::dict());
#endif
  m.def("remesh_planar_patches", &pmp::remesh_planar_patches<Pm>, py::arg("pm"), py::arg("np_in") = py::dict(),
        py::arg("np_out") = py::dict());
  m.def("smooth_shape", &pmp::smooth_shape<Pm>, py::arg("pm"), py::arg("time"), py::arg("np") = py::dict());
  m.def("split_long_edges", &pmp::split_long_edges<Pm>, py::arg("edge_range"), py::arg("max_length"), py::arg("pmesh"),
        py::arg("np") = py::dict());
  // m.def("tangential_relaxation", &pmp::tangential_relaxation<Pm>, // changed in master
  //       py::arg("pm"), py::arg("np") = py::dict());
  m.def("triangulate_faces", &pmp::triangulate_faces<Pm>, py::arg("pm"), py::arg("np") = py::dict());
  m.def("triangulate_faces", &pmp::triangulate_faces_r<Pm>, py::arg("face_range"), py::arg("pm"),
        py::arg("np") = py::dict());
  m.def("triangulate_polygons", &pmp::triangulate_polygons, py::arg("points"), py::arg("polygons"),
        py::arg("np") = py::dict());

  // Corrected Curvature Computation
  m.def("interpolated_corrected_curvatures", &pmp::interpolated_corrected_curvatures<Pm>, py::arg("pm"),
        py::arg("np") = py::dict());
  m.def("interpolated_corrected_curvatures", &pmp::interpolated_corrected_curvatures_v<Pm>, py::arg("v"), py::arg("pm"),
        py::arg("np") = py::dict());

  // Normal Computation
  m.def("compute_face_normal", &pmp::compute_face_normal<Pm>, py::arg("f"), py::arg("pmesh"),
        py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1  // only working for sm
  m.def("compute_face_normals", &pmp::compute_face_normals<Pm, FaceVectorMap>, py::arg("pmesh"),
        py::arg("face_normals"), py::arg("np") = py::dict());
  m.def("compute_normals", &pmp::compute_normals<Pm, VertexVectorMap, FaceVectorMap>, py::arg("vnormals"),
        py::arg("fnormals"), py::arg("pmesh"), py::arg("np") = py::dict());
#endif
  m.def("compute_vertex_normal", &pmp::compute_vertex_normal<Pm>, py::arg("v"), py::arg("pmesh"),
        py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1  // only working for sm
  m.def("compute_vertex_normals", &pmp::compute_vertex_normals<Pm, VertexVectorMap>, py::arg("pmesh"),
        py::arg("vertex_normals"), py::arg("np") = py::dict());
#endif

  // Geometric Measure Functions
  m.def("area", &pmp::area<Pm>, py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("area", &pmp::area_f<Pm>, py::arg("face_range"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("centroid", &pmp::centroid<Pm>, py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("edge_length", &pmp::edge_length<Pm>, py::arg("h"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("face_area", &pmp::face_area<Pm>, py::arg("f"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("face_aspect_ratio", &pmp::face_aspect_ratio<Pm>, py::arg("f"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("face_border_length", &pmp::face_border_length<Pm>, py::arg("f"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("longest_border", &pmp::longest_border<Pm>, py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("match_faces", &pmp::match_faces<Pm>, py::arg("m1"), py::arg("m2"), py::arg("np1") = py::dict(),
        py::arg("np2") = py::dict());
  m.def("squared_edge_length", &pmp::squared_edge_length<Pm>, py::arg("h"), py::arg("tmesh"),
        py::arg("np") = py::dict());
  m.def("squared_face_area", &pmp::squared_face_area<Pm>, py::arg("f"), py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("volume", &pmp::volume<Pm>, py::arg("tmesh"), py::arg("np") = py::dict());

  // Orientation Functions
  m.def("does_bound_a_volume", &pmp::does_bound_a_volume<Pm>,  // TODO: is_cc_outward_oriented
        py::arg("tm"), py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("compatible_orientations", &pmp::compatible_orientations<Pm, FaceBitMap>, py::arg("pm"),
        py::arg("face_bit_map"), py::arg("np") = py::dict());
#endif
  ...
  m.def("is_outward_oriented", &pmp::is_outward_oriented<Pm>, py::arg("tm"), py::arg("np") = py::dict());
  m.def("merge_reversible_connected_components", &pmp::merge_reversible_connected_components<Pm>, py::arg("pm"),
        py::arg("np") = py::dict());
  m.def("orient", &pmp::orient<Pm>, py::arg("tm"), py::arg("np") = py::dict());
  m.def("orient_polygon_soup", &pmp::orient_polygon_soup, py::arg("points"), py::arg("polygons"),
        py::arg("np") = py::dict());
  m.def("orient_to_bound_a_volume", &pmp::orient_to_bound_a_volume<Pm>, py::arg("tm"), py::arg("np") = py::dict());
  m.def("orient_triangle_soup_with_reference_triangle_mesh",
        ...
        py::arg("tm_ref"), py::arg("points"), py::arg("triangles"), py::arg("np1") = py::dict(),
        py::arg("np2") = py::dict());
  // m.def("orient_triangle_soup_with_reference_triangle_soup", &pmp::orient_triangle_soup_with_reference_triangle_soup,
  // // TODO: point_map
  //       py::arg("ref_points"), py::arg("ref_faces"), py::arg("points"), py::arg("faces"), py::arg("np1") =
  //       py::dict(), py::arg("np2") = py::dict());
  m.def("reverse_face_orientations", &pmp::reverse_face_orientations<Pm>, py::arg("face_range"), py::arg("pmesh"));
  m.def("reverse_face_orientations", &PMP::reverse_face_orientations<Pm>, py::arg("pmesh"));
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("volume_connected_components", &pmp::volume_connected_components<Pm, FaceSizeTypeMap>, py::arg("tm"),
        py::arg("volume_id_map"), py::arg("np") = py::dict());
#endif

  // Intersection Functions
  m.def("do_intersect", &pmp::do_intersect_polylines);
  m.def("do_intersect_polyline_ranges", &pmp::do_intersect_polyline_ranges);
  m.def("do_intersect", &pmp::do_intersect_meshes<Pm>, py::arg("pm1"), py::arg("pm2"), py::arg("np1") = py::dict(),
        py::arg("np2") = py::dict());
  m.def("do_intersect", &pmp::do_intersect_mesh_polyline<Pm>, py::arg("pm"), py::arg("lst"),
        py::arg("np") = py::dict());
  m.def("do_intersect_polyline_range", &pmp::do_intersect_mesh_polyline_range<Pm>, py::arg("pm"), py::arg("lst"),
        py::arg("np") = py::dict());
  m.def("does_self_intersect", &pmp::does_self_intersect_faces<Pm>, py::arg("face_range"), py::arg("tmesh"),
        py::arg("np") = py::dict());
  m.def("does_self_intersect", &pmp::does_self_intersect<Pm>, py::arg("pm"), py::arg("np") = py::dict());
  ...
  m.def("intersecting_meshes", &pmp::intersecting_meshes<Pm>, py::arg("range"), py::arg("np") = py::dict(),
        py::arg("nps") = std::vector<py::dict>());
  m.def("self_intersections", &pmp::self_intersections<Pm>, py::arg("pm"), py::arg("np") = py::dict());
  m.def("self_intersections", &pmp::self_intersections_faces<Pm>, py::arg("face_range"), py::arg("pm"),
        py::arg("np") = py::dict());
  m.def("triangle_soup_self_intersections", &pmp::triangle_soup_self_intersections,  // TODO: point_map
        py::arg("points"), py::arg("triangles"), py::arg("np") = py::dict());

  // Combinatorial Repair
  m.def("duplicate_non_manifold_vertices", &pmp::duplicate_non_manifold_vertices<Pm>, py::arg("pmesh"),
        py::arg("np") = py::dict());
  m.def("is_non_manifold_vertex", &PMP::is_non_manifold_vertex<Pm>, py::arg("v"), py::arg("pm"));
  m.def("is_polygon_soup_a_polygon_mesh", &pmp::is_polygon_soup_a_polygon_mesh, py::arg("polygons"));
  m.def("merge_duplicate_points_in_polygon_soup", &pmp::merge_duplicate_points_in_polygon_soup, py::arg("points"),
        py::arg("polygons"), py::arg("np") = py::dict());
  m.def("merge_duplicate_polygons_in_polygon_soup", &pmp::merge_duplicate_polygons_in_polygon_soup, py::arg("points"),
        py::arg("polygons"), py::arg("np") = py::dict());
  ...
  m.def("merge_duplicated_vertices_in_boundary_cycles", &pmp::merge_duplicated_vertices_in_boundary_cycles<Pm>,
        py::arg("pm"), py::arg("np") = py::dict());
  m.def("non_manifold_vertices", &pmp::non_manifold_vertices<Pm>, py::arg("pm"));
  m.def("polygon_mesh_to_polygon_soup", &pmp::polygon_mesh_to_polygon_soup<Pm>, py::arg("pm"),
        py::arg("np") = py::dict());
  ...
  m.def("remove_isolated_points_in_polygon_soup", &pmp::remove_isolated_points_in_polygon_soup, py::arg("points"),
        py::arg("polygons"));
  m.def("repair_polygon_soup", &pmp::repair_polygon_soup, py::arg("points"), py::arg("polygons"),
        py::arg("np") = py::dict());
  m.def("stitch_borders", &pmp::stitch_borders_bc<Pm>, py::arg("boundary_cycle_representatives"), py::arg("pmesh"),
        py::arg("np") = py::dict());
  m.def("stitch_borders", &pmp::stitch_borders_he<Pm>, py::arg("pmesh"), py::arg("hedge_pairs_to_stitch"),
        py::arg("np") = py::dict());
  m.def("stitch_borders", &pmp::stitch_borders<Pm>, py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("stitch_boundary_cycle", &pmp::stitch_boundary_cycle<Pm>, py::arg("h"), py::arg("pmesh"),
        py::arg("np") = py::dict());
  m.def("stitch_boundary_cycles", &pmp::stitch_boundary_cycles<Pm>, py::arg("boundary_cycle_representatives"),
        py::arg("pmesh"), py::arg("np") = py::dict());

  // Geometric Repair
  m.def("remove_almost_degenerate_faces", &pmp::remove_almost_degenerate_faces_r<Pm>, py::arg("face_range"),
        py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("remove_almost_degenerate_faces", &pmp::remove_almost_degenerate_faces<Pm>, py::arg("tmesh"),
        py::arg("np") = py::dict());
  m.def("remove_connected_components_of_negligible_size",
        ...
        py::arg("tmesh"), py::arg("np") = py::dict());
  m.def("remove_isolated_vertices", &PMP::remove_isolated_vertices<Pm>, py::arg("pmesh"));

  // Distance Functions
  m.def("approximate_Hausdorff_distance", &pmp::approximate_Hausdorff_distance<Pm>, py::arg("tm1"), py::arg("tm2"),
        py::arg("np1") = py::dict(), py::arg("np2") = py::dict());
  m.def("approximate_max_distance_to_point_set", &pmp::approximate_max_distance_to_point_set<Pm>, py::arg("tm"),
        py::arg("points"), py::arg("precision"), py::arg("np") = py::dict());
  m.def("approximate_symmetric_Hausdorff_distance", &pmp::approximate_symmetric_Hausdorff_distance<Pm>, py::arg("tm1"),
        py::arg("tm2"), py::arg("np1") = py::dict(), py::arg("np2") = py::dict());
  m.def("bounded_error_Hausdorff_distance", &pmp::bounded_error_Hausdorff_distance<Pm>, py::arg("tm1"), py::arg("tm2"),
        py::arg("error_bound") = 0.0001, py::arg("np1") = py::dict(), py::arg("np2") = py::dict());
  m.def("is_Hausdorff_distance_larger", &pmp::is_Hausdorff_distance_larger<Pm>, py::arg("tm1"), py::arg("tm2"),
        py::arg("distance_bound"), py::arg("error_bound"), py::arg("np1") = py::dict(), py::arg("np2") = py::dict());
  m.def("max_distance_to_triangle_mesh", &pmp::max_distance_to_triangle_mesh<Pm>, py::arg("tm"), py::arg("point"),
        py::arg("np") = py::dict());
  m.def("sample_triangle_mesh", &pmp::sample_triangle_mesh<Pm>, py::arg("tm"), py::arg("np") = py::dict());
  m.def("sample_triangle_soup", &pmp::sample_triangle_soup, py::arg("points"), py::arg("triangles"),
        py::arg("np") = py::dict());

  // Corefinement and Boolean Operations
  m.def("autorefine", &pmp::autorefine<Pm>, py::arg("tm"), py::arg("np") = py::dict());
  m.def("autorefine_triangle_soup", &pmp::autorefine_triangle_soup, py::arg("soup_points"), py::arg("soup_triangles"),
        py::arg("np") = py::dict());
  m.def("clip", &pmp::clip_c<Pm>, py::arg("tm"), py::arg("iso_cuboid"), py::arg("np") = py::dict());
  m.def("clip", &pmp::clip_p<Pm>, py::arg("tm"), py::arg("plane"), py::arg("np") = py::dict());
  m.def("clip", &pmp::clip<Pm>, py::arg("tm"), py::arg("clipper"), py::arg("np_tm") = py::dict(),
        py::arg("np_c") = py::dict());
  m.def("corefine", &pmp::corefine<Pm>, py::arg("pm1"), py::arg("pm2"), py::arg("np1") = py::dict(),
        py::arg("np2") = py::dict());
  m.def("corefine_and_compute_boolean_operations", &pmp::corefine_and_compute_boolean_operations<Pm>, py::arg("pm1"),
        py::arg("pm2"), py::arg("np1") = py::dict(), py::arg("np2") = py::dict(), py::arg("np_out") = py::tuple());
  m.def("corefine_and_compute_difference", &pmp::corefine_and_compute_difference<Pm>, py::arg("tm1"), py::arg("tm2"),
        py::arg("np1") = py::dict(), py::arg("np2") = py::dict(), py::arg("np_out") = py::dict());
  m.def("corefine_and_compute_intersection", &pmp::corefine_and_compute_intersection<Pm>, py::arg("pm1"),
        py::arg("pm2"), py::arg("np1") = py::dict(), py::arg("np2") = py::dict(), py::arg("np_out") = py::dict());
  m.def("corefine_and_compute_union", &pmp::corefine_and_compute_union<Pm>, py::arg("pm1"), py::arg("pm2"),
        py::arg("np1") = py::dict(), py::arg("np2") = py::dict(), py::arg("np_out") = py::dict());
  m.def("split", &pmp::split_c<Pm>, py::arg("tm"), py::arg("iso_cuboid"), py::arg("np") = py::dict());
  m.def("split", &pmp::split_p<Pm>, py::arg("tm"), py::arg("plane"), py::arg("np") = py::dict());
  m.def("split", &pmp::split<Pm>, py::arg("tm"), py::arg("splitter"), py::arg("np_tm") = py::dict(),
        py::arg("np_s") = py::dict());
  m.def("surface_intersection", &pmp::surface_intersection<Pm>, py::arg("tm1"), py::arg("tm2"),
        py::arg("np1") = py::dict(), py::arg("np2") = py::dict());

  // Feature Detection Functions
  m.def("detect_sharp_edges", &pmp::detect_sharp_edges<Pm, edge_bool_map>, py::arg("pm"), py::arg("angle_in_deg"),
        py::arg("edge_is_feature_map"), py::arg("np") = py::dict());
  // only for sm
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("sharp_edges_segmentation",
        &pmp::sharp_edges_segmentation<Pm, Pm::Property_map<Ed, bool>, Pm::Property_map<Fd, int>>, py::arg("pmesh"),
        py::arg("angle_in_deg"), py::arg("edge_is_feature_map"), py::arg("patch_id_map"), py::arg("np") = py::dict());
  m.def("detect_vertex_incident_patches",
        ...
        py::arg("pmesh"), py::arg("patch_id_map"), py::arg("vertex_incident_patches_map"), py::arg("np") = py::dict());
#endif

  // Location Functions
  m.def("barycentic_coordinates", &pmp::barycentic_coordinates, py::arg("p"), py::arg("q"), py::arg("r"),
        py::arg("query"));
  m.def("get_descriptor_from_location", &pmp::get_descriptor_from_location<Pm>, py::arg("loc"), py::arg("tm"));
  m.def("is_in_face", &pmp::is_in_face_bar<Pm>, py::arg("bar"), py::arg("tm"));
  m.def("is_on_face_border", &pmp::is_on_face_border<Pm>, py::arg("loc"), py::arg("tm"));
  m.def("is_on_halfedge", &pmp::is_on_halfedge<Pm>, py::arg("loc"), py::arg("hd"), py::arg("tm"));
  m.def("is_on_mesh_border", &pmp::is_on_mesh_border<Pm>, py::arg("loc"), py::arg("tm"));
  m.def("is_on_vertex", &pmp::is_on_vertex<Pm>, py::arg("loc"), py::arg("vd"), py::arg("tm"));
  ...
  m.def("is_cap_triangle_face", &pmp::is_cap_triangle_face<Pm>, py::arg("f"), py::arg("tm"), py::arg("threshold"),
        py::arg("np") = py::dict());
  m.def("is_degenerate_edge", &pmp::is_degenerate_edge<Pm>, py::arg("e"), py::arg("pm"), py::arg("np") = py::dict());
  m.def("is_degenerate_triangle_face", &pmp::is_degenerate_triangle_face<Pm>, py::arg("f"), py::arg("tm"),
        py::arg("np") = py::dict());
  m.def("is_needle_triangle_face", &pmp::is_needle_triangle_face<Pm>, py::arg("f"), py::arg("tm"), py::arg("threshold"),
        py::arg("np") = py::dict());

  // other
  m.def("add_bbox", &pmp::add_bbox<Pm>, py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("bbox", &pmp::bbox<Pm>, py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("border_halfedges", &pmp::border_halfedges<Pm>, py::arg("face_range"), py::arg("pm"),
        py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("detect_corners_of_regions", &pmp::detect_corners_of_regions<Pm, RegionMap, CornerIdMap>, py::arg("pmesh"),
        py::arg("region_map"), py::arg("nb_regions"), py::arg("corner_id_map"), py::arg("np") = py::dict());
#endif
  m.def("edge_bbox", &pmp::edge_bbox<Pm>, py::arg("ed"), py::arg("pmesh"), py::arg("np") = py::dict());
  m.def("extract_boundary_cycles", &pmp::extract_boundary_cycles<Pm>, py::arg("pmesh"));
  m.def("face_bbox", &pmp::face_bbox<Pm>, py::arg("f"), py::arg("pmesh"), py::arg("np") = py::dict());
#if CGALPY_PMP_POLYGONAL_MESH == 1
  m.def("refine_mesh_at_isolevel", &pmp::refine_mesh_at_isolevel<Pm, ValueMap>, py::arg("pm"), py::arg("value_map"),
        py::arg("isovalue"), py::arg("np") = py::dict());
  m.def("region_growing_of_planes_on_faces", &pmp::region_growing_of_planes_on_faces<Pm, RegionMap>, py::arg("pmesh"),
        py::arg("region_map"), py::arg("np") = py::dict());
#endif
  ...
  m.def("set_start_handling_intersection_of_coplanar_faces",
        &pmp::set_start_handling_intersection_of_coplanar_faces_fn);
  ...
  py::class_<Pe>(m, "Polyhedral_envelope")
      .def(py::init<Pm&, double, const py::dict&>(), py::arg("tmesh"), py::arg("epsilon"), py::arg("np") = py::dict())
      .def(py::init<const std::vector<Fd>&, Pm&, double, const py::dict&>(), py::arg("face_range"), py::arg("tmesh"),
           py::arg("epsilon"), py::arg("np") = py::dict())
      ...
      .def(
          "inside", [](const Pe& i, const Point_3& query) { return i(query); }, py::arg("query"))
      .def(
          "inside", [](const Pe& i, const Point_3& source, const Point_3& target) { return i(source, target); },
          py::arg("source"), py::arg("target"))
      .def(
          "inside", [](const Pe& i, const Point_3& t0, const Point_3& t1, const Point_3& t2) { return i(t0, t1, t2); },
  ...
  py::class_<Asf>(m, "Adaptive_sizing_field")
      .def(py::init<FT, const std::tuple<FT, FT>&, const std::vector<face_descriptor>&, Pm&, const py::dict&>(),
           py::arg("tol"), py::arg("edge_len_min_max"), py::arg("face_range"), py::arg("pmesh"),
           py::arg("np") = py::dict())
      .def("at", &Asf::at)
  ...
  py::class_<Dv>(m, "Default_visitor").def(py::init<>());

  using Dov = pmp::Default_orientation_visitor;
  py::class_<Dov>(m, "Default_orientation_visitor").def(py::init<>());
  ...
  py::class_<Hfv>(m, "Hole_filling_default_visitor").def(py::init<>());

  // corefine
  using Cv = pmp::Corefine_visitor<Pm>;
  py::class_<Cv>(m, "Corefine_visitor").def(py::init<>());
}
