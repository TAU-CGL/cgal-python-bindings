...
/*! The implementation of the bindings of the
 * CGAL::Graphics_scene_options class template presents a specific challenge:
 * Python callbacks must remain alive after their setter returns and until a
 * later graphics-scene operation invokes them.
 *
 * Graphics_scene_options_extended derives from CGAL::Graphics_scene_options
 * and retains the Python callback objects explicitly so Python's cyclic GC can
 * traverse and clear them. The C++ adapter lambdas are installed directly in
 * the corresponding std::function callback members inherited from
 * CGAL::Graphics_scene_options, avoiding a second heap-allocated wrapper
 * ownership layer.
 *
 * The Python user should normally use Graphics_scene_options_extended, which
 * is bound as "Graphics_scene_options". The CGAL base class is exposed as
 * "Graphics_scene_options_base".
 */
