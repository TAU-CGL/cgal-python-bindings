...
void export_2d_range_and_neighbor_search(py::module_& m) {
  ...
  using Tri_2 = tri2::Triangulation_2;

  if (! add_attr<Tricc2>(m, "Triangulation_cw_ccw_2")) {
    py::class_<Tricc2> cw_ccw_c(m, "Triangulation_cw_ccw_2");
    tri2::export_triangulation_cw_ccw_2<Tri_2>(cw_ccw_c);
  }

  if (! add_attr<Tri_2>(m, "Triangulation_2")) {
    py::class_<Tri_2, Tricc2> tri_c(m, "Triangulation_2");
    tri2::export_triangulation_2<Tri_2>(tri_c);
  }
}
