...
void register_map(py::module_& m, const std::string& map_name) {
  py::class_<Map_type>(m, map_name.c_str())
      ...
      .def(
          "put",
          [](Map_type& p, const typename Map_type::key_type& k, const typename Map_type::value_type& v) {
            put(p, k, v);
          },
      ...
      .def(
          ...
          py::arg("key"), py::arg("value"))
      .def("get", [](const Map_type& p, const typename Map_type::key_type& k) { return get(p, k); })
      .def("__getitem__", [](const Map_type& p, const typename Map_type::key_type& k) { return get(p, k); })
...
// Access functions
const Vertex& vertex(const Halfedge& e) {
  return (*(e.vertex()));
}
const Halfedge& opposite(const Halfedge& e) {
  return (*(e.opposite()));
}
const Halfedge& next(const Halfedge& e) {
  return (*(e.next()));
}
const Halfedge& prev(const Halfedge& e) {
  return (*(e.prev()));
}
const Halfedge& halfedge(const Face& f) {
  return (*(f.halfedge()));
}

// Read a surface mesh from a file.
template <typename Polyhedron_3>
Polyhedron_3 read_polygon_mesh(const std::string& filename, const py::dict& np = py::dict()) {
  using Prn = Polyhedron_3;

  Prn pol;
  if (!CGAL::IO::read_polygon_mesh(filename, pol, internal::parse_named_parameters(np)))
...
template <typename Polyhedron_3>
bool write_polygon_mesh(std::string fname, const Polyhedron_3& pm, const py::dict& np = py::dict()) {
  return CGAL::IO::write_polygon_mesh(fname, pm, internal::parse_named_parameters(np));
}

// Draw a polyhedron.
#ifdef CGALPY_HAS_VISUAL
void draw(const Polyhedron_3& prn) {
  CGAL::draw(prn);
}
#endif
...
auto make_triangle(Polyhedron_3& prn,
                   const typename Polyhedron_3::Point& p1,
...
auto my_vertices(const Polyhedron_3& prn) {
  return py::make_iterator(py::type<Polyhedron_3::Vertex_const_iterator>(), "Vertex_iterator", prn.vertices_begin(),
                           prn.vertices_end());
}
//
auto my_halfedges(const Polyhedron_3& prn) {
  return py::make_iterator(py::type<Polyhedron_3::Halfedge_const_iterator>(), "Halfedge_iterator",
                           prn.halfedges_begin(), prn.halfedges_end());
}

//
auto my_edges(const Polyhedron_3& prn) {
  return py::make_iterator(py::type<Polyhedron_3::Edge_const_iterator>(), "Edge_iterator", prn.edges_begin(),
                           prn.edges_end());
}

//
auto my_faces(const Polyhedron_3& prn) {
  return py::make_iterator(py::type<Polyhedron_3::Face_const_iterator>(), "Face_iterator", prn.facets_begin(),
                           prn.facets_end());
}

//
auto my_planes(const Polyhedron_3& prn) {
  return py::make_iterator(py::type<Polyhedron_3::Plane_const_iterator>(), "Plane_iterator", prn.planes_begin(),
                           prn.planes_end());
}
...
void export_vertex(C& prn_c) {
  ...
  vertex_c.def(py::init<>())
      .def(
          "point", [](const Vertex& v) { return v.point(); }, ri)
...
void export_vertex_handle(C& prn_c) {
  ...
  if (add_attr<Vertex_handle>(prn_c, "Vertex_handle"))
    return;
...
void export_face(C& prn_c) {
  ...
  face_c.def(py::init<>())
      .def(
          "plane", [](const Face& f) { return f.plane(); }, ri)
...
void export_face_handle(C& prn_c) {
  ...
  if (add_attr<Face_handle>(prn_c, "Face_handle"))
    return;
...
void export_polyhedron_3(py::module_& m) {
...
#endif
  m.def("read_polygon_mesh", &pol3::read_polygon_mesh<Prn>, py::arg("filename"), py::arg("np") = py::dict());
  m.def("write_polygon_mesh", &pol3::write_polygon_mesh<Prn>, py::arg("filename"), py::arg("pm"),
        py::arg("np") = py::dict());
  ...
  pol3::vertex_map<Prn, pcad>(m, "vertex_principal_curvatures_and_directions_map", "dynamic_property_vertex_PC");

  m.def("get_edge_is_feature_map", [](const Prn& sm) { return get(CGAL::edge_is_feature, sm); });

  py::class_<CGAL::Polyhedron_is_feature_edge_pmap>(m, "Polyhedron_is_feature_edge_pmap").def(py::init<>());

  using poly_items = pol3::Polyhedron_items;
  using poly_items_derived = CGAL::I_Polyhedron_derived_items_3<poly_items>;

  // the type
  using vertex_point_map_type = CGAL::internal::Point_accessor<
      CGAL::internal::In_place_list_iterator<
          CGAL::HalfedgeDS_in_place_list_vertex<CGAL::I_Polyhedron_vertex<
              CGAL::HalfedgeDS_vertex_base<CGAL::HalfedgeDS_list_types<Kernel, poly_items_derived, std::allocator<int>>,
                                           std::integral_constant<bool, true>, Point_3>>>,
          std::allocator<CGAL::HalfedgeDS_in_place_list_vertex<CGAL::I_Polyhedron_vertex<
              CGAL::HalfedgeDS_vertex_base<CGAL::HalfedgeDS_list_types<Kernel, poly_items_derived, std::allocator<int>>,
                                           std::integral_constant<bool, true>, Point_3>>>>>,
      Point_3, Point_3 const&, true>;
  py::class_<vertex_point_map_type>(m, "vertex_point_map").def(py::init<>());
  ...
  m.def("is_valid_vertex_descriptor", &boost_utils::my_is_valid_vertex_descriptor<Prn>, py::arg("v"), py::arg("g"),
        py::arg("verbose") = false);
  m.def("is_valid_halfedge_descriptor", &boost_utils::my_is_valid_halfedge_descriptor<Prn>, py::arg("h"), py::arg("g"),
        py::arg("verbose") = false);
  m.def("is_valid_edge_descriptor", &boost_utils::my_is_valid_edge_descriptor<Prn>, py::arg("e"), py::arg("g"),
        py::arg("verbose") = false);
  m.def("is_valid_face_descriptor", &boost_utils::my_is_valid_face_descriptor<Prn>, py::arg("f"), py::arg("g"),
        py::arg("verbose") = false);
