...
void export_kinetic_surface_reconstruction(py::module_& m) {
  ...
  py::class_<KSR> ksr(m, "Kinetic_surface_reconstruction_3",
                      "Pipeline for piecewise planar surface reconstruction from a point cloud via inside/outside "
                      "labeling of a kinetic partition using min-cut.\n");
  ksr.def(
         "__init__",
         [](KSR& ksr, Point_range& points, const py::dict& np = py::dict()) {
           ...
           if (np.contains("minimum_region_size")) {
             ...
             new (&ksr) KSR(points, internal::parse_named_parameters(np).minimum_region_size(minimum_region_size));
           } else {
             new (&ksr) KSR(points, internal::parse_named_parameters(np));
           }
         },
         py::arg("points"), py::arg("np") = py::dict(), "creates a Kinetic_shape_reconstruction_3 object.\n")
      .def(
          "detect_planar_shapes",
          [](KSR& ksr, const py::dict& np = py::dict()) {
            return ksr.detect_planar_shapes(internal::parse_named_parameters(np));
          },
          py::arg("np") = py::dict(), "detects shapes in the provided point cloud and regularizes them.\n")
      .def(
          "detected_planar_shapes", [](KSR& ksr) { return ksr.detected_planar_shapes(); },
          "returns the support planes of the detected and regularized shapes.\n")
      .def(
          "detected_planar_shape_indices", [](KSR& ksr) { return ksr.detected_planar_shape_indices(); },
      ...
      .def(
          ...
          py::arg("np") = py::dict(), "initializes the kinetic partition.\n")
      // .def("kinetic_partition", [](const KSR& ksr) { // deprecated?
      //   return ksr.kinetic_partition();
      // },
      // "gives access to the kinetic partition.\n")
      .def(
          "partition", [](KSR& ksr, std::size_t k) { return ksr.partition(k); }, py::arg("k"),
          "propagates the kinetic polygons in the initialized partition.\n")
      .def(
          "reconstruct",
          [](KSR& ksr, FT lambda, std::map<KSP::Face_support, bool> external_nodes) {
          ...
          "uses min-cut to solve an inside/outside labeling of the volumes of the kinetic partition and provides the "
          "reconstructed surface as a list of indexed polygons.\n"
      ...
      .def(
          ...
          "uses min-cut to solve an inside/outside labeling of the volumes of the kinetic partition and provides the "
          "reconstructed surface as a list of indexed polygons.\n"
          "Estimates a horizontal ground plane within the detected shapes. Cells in the partition below the ground "
          "plane receive a weight to be labeled as inside. The z axis is considered as vertical upwards pointing.\n");

  py::enum_<KSR::KSP::Face_support>(
      ksr, "Face_support",
      "identifies the support of a face in the exported linear cell complex, which is either an input polygon, "
      "identified by a non-negative number, a side of the bounding box in the rotated coordinate system or a face of "
      "the octree used to partition the scene.")
