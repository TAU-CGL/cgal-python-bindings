...
namespace pp2_doc = cgalpy::pp2::docstrings;
...
bool y_monotone_partition_is_valid_2(const Polygon_2& p, const py::list& lst) {
  ...
  auto sbegin = py_list_forward_iterator<Polygon_2>(lst);
  auto send = py_list_forward_iterator<Polygon_2>(lst, false);
...
void export_polygon_partition_2(py::module_& m) {
  ...
  m.def("is_y_monotone_2", &cgalpy::pp2::is_y_monotone_2,
        py::arg("polygon"), pp2_doc::is_y_monotone_2);
  m.def("is_convex_2", &cgalpy::pp2::is_convex_2,
        py::arg("polygon"), pp2_doc::Is_convex_2_operator_call);
