...
#ifndef CGAL_PYTHON_DOCSTRINGS_BOUNDING_VOLUMES_H
#define CGAL_PYTHON_DOCSTRINGS_BOUNDING_VOLUMES_H
...
constexpr const char* ApproximateMinEllipsoid_d_Traits_d__class__ = "This concept defines the requirements for traits classes of CGAL::Approximate_min_ellipsoid_d <Traits> .";

constexpr const char* ApproximateMinEllipsoid_d_Traits_d_FT = "typedef double FT

Returns: unspecified_type";
constexpr const char* ApproximateMinEllipsoid_d_Traits_d_ET = "Some model of concept RingNumberType that provides exact arithmetic.

In addition, ET must be able to exactly represent any finite double value. (An
example for such a type is CGAL::MP_Float .). The type ET is to be used by the
Approximate_min_ellipsoid_d<Traits> class for internal, exact computations.

Returns: unspecified_type";
constexpr const char* ApproximateMinEllipsoid_d_Traits_d_Point = "Type of the input points.

Point must provide the default and copy constructor, and must be a model of
DefaultConstructible , CopyConstructible , and Assignable .

Returns: unspecified_type";
constexpr const char* ApproximateMinEllipsoid_d_Traits_d_Cartesian_const_iterator = "Model for the STL concept RandomAccessIterator whose value type must be convertible to double .

This type is used to iterate over the Cartesian coordinates of an instance of
type Point , see cartesian_begin() below.

Returns: unspecified_type";
constexpr const char* ApproximateMinEllipsoid_d_Traits_d_dimension = "returns the dimension of a point p .
...
Returns: int";
constexpr const char* ApproximateMinEllipsoid_d_Traits_d_cartesian_begin = "returns an input iterator over the Euclidean coordinates of the point $ p$ .
...
happen that the computed approximation ellipsoid has a worse approximation
ratio (and $ \\epsilon$ can thus be larger than eps in general). In any case,
the number $ \\epsilon$ (and with this, the achieved approximation $
1+\\epsilon$ ) can be queried by calling the routine achieved_epsilon()
discussed below. Iterator must be a model of InputIterator with Point as value
type. The dimension $ d$ of the input points must be at least $ 2$ , and $
\\epsilon>0$ .
...
\\epsilon'$ is the number returned by achieved_epsilon() . $ 0\\leq i,j\\leq d$ ,
where $ d$ is the dimension of the points $ P$ , and ame.is_full_dimensional()
== true .
...
\\epsilon'$ is the number returned by achieved_epsilon() . $ 0\\leq i\\leq d$ ,
where $ d$ is the dimension of the points $ P$ , and ame.is_full_dimensional()
== true .
...
$ \\epsilon'$ is the number returned by achieved_epsilon() .
...
involved relative error. (See also method axis_direction_cartesian_begin() .)
...
w.r.t. the involved relative error. An approximation to the length of axis $
i$ is given by the $ i$ th entry of axes_lengths_begin() .
...
false means that the points $ P$ are contained in a \"very thin\" linear
subspace of $ \\E^d$ , and as a consequence, the algorithm cannot compute an
approximation. More precisely, a return value of false means that the points $
P$ are contained between two parallel hyperplanes in $ \\E^d$ that are very
close to each other (possibly at distance zero) - so close, that the algorithm
could not compute an approximation ellipsoid. Similarly, a return value of
true does not guarantee $ P$ to be full-dimensional; but there exists an input
pointset $ P'$ such that the points $ P'$ and $ P$ have almost identical
...
equal to min_annulus.ambient_dimension() .
...
necessary, however, the algorithm's efficiency depends on the order in which
the points are processed, and a bad order might lead to extremely poor
performance (see example below). InputIterator must be a model of
InputIterator with Point as value type.
...
i< $ min_circle.number_of_support_points() .
...
necessary, however, the algorithm's efficiency depends on the order in which
the points are processed, and a bad order might lead to extremely poor
performance (see example below). InputIterator is a model of InputIterator
with Point as value type.
...
i<$ min_ellipse.number_of_support_points() .
...
is the set of points $ (hx,hy,hw)$ satisfying $
...
counterclockwise), op(p1,d1,p2,d2,p3,p4) returns the parallelogram with one of
...
type. If the traits parameter is not supplied, the class Traits must provide a
default constructor. All points have the same dimension.
...
recomputing the smallest enclosing sphere from scratch. The dimension of p
equals ambient_dimension() if min_sphere is not empty.
...
the number $ a+b\\sqrt{t}$ , where $ a$ is the first and $ b$ the second
element of the pair and where the number $ t$ is accessed using the member
...
(Normally, Algorithm coincides with Traits::Algorithm . However, if the method
Traits::Algorithm should not be supported anymore in a future release, then
...
not supplied, the class Traits must provide a default constructor.
...
$ a+b\\sqrt{t}$ , where $ t$ is the discriminant of the minsphere as returned
...
the first and $ b$ the second component of the pair returned by radius() .
...
intersection of the vertical line through $ p$ and the horizontal line through
$ q$ .
...
// Circle
constexpr const char* Circle__class__ = "An object circle of the class Circle is a circle in the two-dimensional Euclidean plane $ \\E^2$ . Its boundary splits the plane into a bounded and an unbounded side. By definition, an empty circle has no boundary and no bounded side, i.e. its unbounded side equals the whole plane $ \\E^2$ . A circle containing exactly one point $ p$ has no bounded side, its boundary is $ \\{p\\}$ , and its unbounded side equals $ \\E^2 \\setminus \\{p\\}$ .";

constexpr const char* Circle_Point = "Point type.

Returns: unspecified_type";
constexpr const char* Circle_Distance = "Distance type.

The function squared_radius() (see below) returns an object of this type. Only
needed, if the member function is_valid() of Min_circle_2 is used.

Returns: unspecified_type";
constexpr const char* Circle_set = "sets circle to the empty circle.";
constexpr const char* Circle_set_1 = "sets circle to the circle containing exactly p .

Parameters
----------
p : const Point &";
constexpr const char* Circle_set_2 = "sets circle to the circle with diameter equal to the segment connecting p and q .
...
p : const Point &
q : const Point &";
constexpr const char* Circle_set_3 = "sets circle to the circle through p , q , and r .

The algorithm guarantees that set() is never called with three collinear
points.
...
q : const Point &
r : const Point &";
constexpr const char* Circle_has_on_unbounded_side = "returns true , iff p lies properly outside of circle .
...
Returns: bool";
constexpr const char* Circle_bounded_side = "returns CGAL::ON_BOUNDED_SIDE , CGAL::ON_BOUNDARY , or CGAL::ON_UNBOUNDED_SIDE iff p lies properly inside, on the boundary, or properly outside of circle , resp.
...
Returns: CGAL::Bounded_side";
constexpr const char* Circle_has_on_bounded_side = "returns true , iff p lies properly inside circle .
...
Returns: bool";
constexpr const char* Circle_has_on_boundary = "returns true , iff p lies on the boundary of circle .
...
Returns: bool";
constexpr const char* Circle_is_empty = "returns true , iff circle is empty (this implies degeneracy).

Only needed, if the corresponding predicate of Min_circle_2 is used.

Returns: bool";
constexpr const char* Circle_is_degenerate = "returns true , iff circle is degenerate, i.e. if circle is empty or equal to a single point.

Only needed, if the corresponding predicate of Min_circle_2 is used.

Returns: bool";
constexpr const char* Circle_operator = "returns true , iff circle and circle2 are equal.
...
Returns: bool";
constexpr const char* Circle_center = "returns the center of circle .

Only needed, if the member function is_valid() of Min_circle_2 is used.

Returns: Point";
constexpr const char* Circle_squared_radius = "returns the squared radius of circle .

Only needed, if the member function is_valid() of Min_circle_2 is used.

Returns: Distance";
constexpr const char* Circle_operator_1 = "writes circle to output stream os .
...
// Ellipse
constexpr const char* Ellipse__class__ = "An object ellipse of the class Ellipse is an ellipse in two-dimensional Euclidean plane $ \\E^2$ . Its boundary splits the plane into a bounded and an unbounded side. By definition, an empty ellipse has no boundary and no bounded side, i.e. its unbounded side equals the whole plane $ \\E^2$ .";

constexpr const char* Ellipse_Point = "Point type.

Returns: unspecified_type";
constexpr const char* Ellipse_set = "sets ellipse to the empty ellipse.";
constexpr const char* Ellipse_set_1 = "sets ellipse to the ellipse containing exactly p .

Parameters
----------
p : const Point &";
constexpr const char* Ellipse_set_2 = "sets ellipse to the ellipse containing exactly the segment connecting p and q .
...
p : const Point &
q : const Point &";
constexpr const char* Ellipse_set_3 = "sets ellipse to the smallest ellipse through p , q , r .
...
q : const Point &
r : const Point &";
constexpr const char* Ellipse_set_4 = "sets ellipse to the smallest ellipse through p , q , r , s .
...
r : const Point &
s : const Point &";
constexpr const char* Ellipse_set_5 = "sets ellipse to the unique conic through p , q , r , s , t .
...
s : const Point &
t : const Point &";
constexpr const char* Ellipse_has_on_unbounded_side = "returns true , iff p lies properly outside of ellipse .
...
Returns: bool";
constexpr const char* Ellipse_bounded_side = "returns CGAL::ON_BOUNDED_SIDE , CGAL::ON_BOUNDARY , or CGAL::ON_UNBOUNDED_SIDE iff p lies properly inside, on the boundary, or properly outside of ellipse , resp.
...
Returns: CGAL::Bounded_side";
constexpr const char* Ellipse_has_on_bounded_side = "returns true , iff p lies properly inside ellipse .
...
Returns: bool";
constexpr const char* Ellipse_has_on_boundary = "returns true , iff p lies on the boundary of ellipse .
...
Returns: bool";
constexpr const char* Ellipse_is_empty = "returns true , iff ellipse is empty (this implies degeneracy).

Only needed, if the corresponding predicate of CGAL::Min_ellipse_2 is used.

Returns: bool";
constexpr const char* Ellipse_is_degenerate = "returns true , iff ellipse is degenerate, i.e. if ellipse is empty or equal to a single point.

Only needed, if the corresponding predicate of CGAL::Min_ellipse_2 is used.

Returns: bool";
constexpr const char* Ellipse_operator = "writes ellipse to output stream os .
...
// MinCircle2Traits
constexpr const char* MinCircle2Traits__class__ = "This concept defines the requirements for traits classes of CGAL::Min_circle_2 <Traits> .";

constexpr const char* MinCircle2Traits_Point = "The point type must provide default and copy constructor, assignment and equality test.

Returns: unspecified_type";
constexpr const char* MinCircle2Traits_Circle = "The circle type must fulfill the requirements listed below in the next section.

Returns: unspecified_type";
constexpr const char* MinCircle2Traits_circle = "The current circle.

This variable is maintained by the algorithm, the user should neither access
nor modify it directly.

Returns: Circle";
constexpr const char* MinCircle2Traits_orientation = "returns constants CGAL::LEFT_TURN , CGAL::COLLINEAR , or CGAL::RIGHT_TURN iff r lies properly to the left of, on, or properly to the right of the oriented line through p and q , resp.
...
// MinEllipse2Traits
constexpr const char* MinEllipse2Traits__class__ = "This concept defines the requirements for traits classes of CGAL::Min_ellipse_2 <Traits> .";

constexpr const char* MinEllipse2Traits_Point = "The point type must provide default and copy constructor, assignment and equality test.

Returns: unspecified_type";
constexpr const char* MinEllipse2Traits_Ellipse = "The ellipse type must fulfill the requirements listed below in the next section.

Returns: unspecified_type";
constexpr const char* MinEllipse2Traits_ellipse = "The current ellipse.
...
constexpr const char* MinQuadrilateralTraits_2__class__ = "The concept MinQuadrilateralTraits_2 defines types and operations needed to compute minimum enclosing quadrilaterals of a planar point set using the functions min_rectangle_2() , min_parallelogram_2() and min_strip_2() .";

constexpr const char* MinQuadrilateralTraits_2_Point_2 = "type for representing points.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Vector_2 = "type for representing vectors.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Direction_2 = "type for representing directions.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Line_2 = "type for representing lines.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Rectangle_2 = "type for representing (not necessarily axis-parallel) rectangles.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Parallelogram_2 = "type for representing parallelograms.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Strip_2 = "type for representing strips, that is the closed region bounded by two parallel lines.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Equal_2 = "a model for Kernel::Equal_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Less_xy_2 = "a model for Kernel::LessXY_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Less_yx_2 = "a model for Kernel::LessYX_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Has_on_negative_side_2 = "a model for Kernel::HasOnNegativeSide_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Compare_angle_with_x_axis_2 = "a model for Kernel::CompareAngleWithXAxis_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Area_less_rectangle_2 = "AdaptableFunctor op : Rectangle_2 $ \\times$ Rectangle_2 $ \\rightarrow$ bool .

op(r1,r2) returns true , iff the area of $ r1$ is strictly less than the area
of $ r2$ .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Area_less_parallelogram_2 = "AdaptableFunctor op : Parallelogram_2 $ \\times$ Parallelogram_2 $ \\rightarrow$ bool .

op(p1,p2) returns true , iff the area of $ p1$ is strictly less than the area
of $ p2$ .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Width_less_strip_2 = "AdaptableFunctor op : Strip_2 $ \\times$ Strip_2 $ \\rightarrow$ bool .

op(s1,s2) returns true , iff the width of $ s1$ is strictly less than the
width of $ s2$ .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Orientation_2 = "a model for Kernel::Orientation_2 .

The following type is used for expensive precondition checking only.

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_vector_2 = "a model for Kernel::ConstructVector_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_vector_from_direction_2 = "AdaptableFunctor.

op : Direction_2 $ \\rightarrow$ Vector_2 . op(d) returns a vector in direction
d .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_perpendicular_vector_2 = "a model for Kernel::ConstructPerpendicularVector_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_direction_2 = "a model for Kernel::ConstructDirection_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_opposite_direction_2 = "a model for Kernel::ConstructOppositeDirection_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_line_2 = "a model for Kernel::ConstructLine_2 .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_rectangle_2 = "Function class op : Point_2 $ \\times$ Direction_2 $ \\times$ Point_2 $ \\times$ Point_2 $ \\times$ Point_2 $ \\rightarrow$ Rectangle_2 .
...
Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_parallelogram_2 = "Function class.
...
Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_Construct_strip_2 = "Function class op : Point_2 $ \\times$ Direction_2 $ \\times$ Point_2 $ \\rightarrow$ Strip_2 .

op(p1,d,p2) returns the strip bounded by the lines through p1 resp. p2 with
direction d .

Returns: unspecified_type";
constexpr const char* MinQuadrilateralTraits_2_copy_rectangle_vertices_2 = "copies the four vertices of r in counterclockwise order to o .
...
Returns: OutputIterator";
constexpr const char* MinQuadrilateralTraits_2_copy_parallelogram_vertices_2 = "copies the four vertices of p in counterclockwise order to o .
...
Returns: OutputIterator";
constexpr const char* MinQuadrilateralTraits_2_copy_strip_lines_2 = "copies the two lines bounding s to o .
...
constexpr const char* MinSphereAnnulusDTraits__class__ = "This concept defines the requirements for traits classes of $ d$ -dimensional min sphere and min annulus algorithms.";

constexpr const char* MinSphereAnnulusDTraits_Point_d = "point type used to represent the input points.

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_Rep_tag = "compile time tag to distinguish between Cartesian and homogeneous representation of the input points.

Rep_tag has to be either CGAL::Cartesian_tag or CGAL::Homogeneous_tag .

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_RT = "number type used to represent the coordinates of the input points.

It has to be a model for RingNumberType .

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_FT = "number type used to return either the squared radius of the smallest enclosing sphere or annulus, or the squared distance of the polytopes.
...
Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_Access_dimension_d = "data accessor object used to access the dimension of the input points.

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_Access_coordinates_begin_d = "data accessor object used to access the coordinates of the input points.

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_Construct_point_d = "constructor object used to construct either the center of the smallest enclosing sphere or annulus, or the points realizing the distance between the two polytopes.

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_ET = "exact number type used to do the exact computations in the underlying solver for linear programs.

It has to be a model for RingNumberType . There must be an implicit conversion
from RT to ET available.

Returns: unspecified_type";
constexpr const char* MinSphereAnnulusDTraits_NT = "fast (possibly inexact) number type used to speed up the pricing step in the underlying solver for linear programs.
...
constexpr const char* MinSphereOfSpheresTraits__class__ = "A model of concept MinSphereOfSpheresTraits must provide the following constants, types, predicates and operations.";

constexpr const char* MinSphereOfSpheresTraits_Sphere = "is a typedef to some class representing a sphere.

(The package will compute the minsphere of spheres of type Sphere .) The type
Sphere must provide a copy constructor.

Returns: unspecified_type";
constexpr const char* MinSphereOfSpheresTraits_FT = "is a (exact or inexact) field number type.

FT must either be double or float , or an exact field number type. (An exact
number type is one which evaluates arithmetic expressions involving the four
basic operations and comparisons with infinite precision, that is, like in $
\\mathbb{R}$ .)

Returns: unspecified_type";
constexpr const char* MinSphereOfSpheresTraits_Cartesian_const_iterator = "non-mutable model of the STL concept ForwardIterator with value type FT .

Used to access the center coordinates of a sphere.

Returns: unspecified_type";
constexpr const char* MinSphereOfSpheresTraits_Use_square_roots = "must typedef to either CGAL::Tag_true or CGAL::Tag_false .

The algorithm uses (depending on the type MinSphereOfSpheresTraits::Algorithm
) floating-point arithmetic internally for some intermediate computations. The
type Use_square_roots affects how these calculations are done: When
Use_square_roots is Tag_true , the algorithm computing the minsphere will
perform square-root operations on double s and float s where appropriate. On
the other hand, if Use_square_roots is CGAL::Tag_false , the algorithm will
work without doing square-roots. Note: On some platforms the algorithm is much
faster when square-roots are disabled (due to lacking hardware support).

Returns: unspecified_type";
constexpr const char* MinSphereOfSpheresTraits_Algorithm = "selects the method to compute the minsphere with.

It must typedef to either CGAL::Default_algorithm , CGAL::LP_algorithm or
CGAL::Farthest_first_heuristic . The recommended choice is the first, which is
a synonym to the one of the other two methods which we consider \"the best in
practice.\" In case of CGAL::LP_algorithm , the minsphere will be computed
using the LP-algorithm [11] , which in our implementation has maximal expected
running time $O(2^d n)$ (in the number of operations on the number type FT ).
In case of CGAL::Farthest_first_heuristic , a simple heuristic will be used
instead which seems to work fine in practice, but comes without a guarantee on
the running time. For an inexact number type FT we strongly recommend
CGAL::Default_algorithm , or, if you want, CGAL::Farthest_first_heuristic ,
since these handle most degeneracies in a satisfying manner. Notice that this
compile-time flag is taken as a hint only. Should one of the methods not be
available anymore in a future release, then the default algorithm will be
chosen.

Returns: unspecified_type";
constexpr const char* MinSphereOfSpheresTraits_D = "specifies the dimension of the spheres you want to compute the minsphere of.

Returns: const int";
constexpr const char* MinSphereOfSpheresTraits_radius = "returns the radius of sphere s .
...
Returns: FT";
constexpr const char* MinSphereOfSpheresTraits_center_cartesian_begin = "returns an iterator referring to the first of the D Cartesian coordinates of the center of s .
...
constexpr const char* RectangularPCenterTraits_2__class__ = "The concept RectangularPCenterTraits_2 defines types and operations needed to compute rectilinear $ p$ -centers of a planar point set using the function CGAL::rectangular_p_center_2() .";

constexpr const char* RectangularPCenterTraits_2_FT = "model for FieldNumberType .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Point_2 = "model for Kernel::Point_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Iso_rectangle_2 = "model for Kernel::IsoRectangle_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Less_x_2 = "model for Kernel::LessX_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Less_y_2 = "model for Kernel::LessY_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_vertex_2 = "model for Kernel::ConstructVertex_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_iso_rectangle_2 = "model for Kernel::ConstructIsoRectangle_2 .

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Signed_x_distance_2 = "adaptable binary function class: Point_2 $ \\times$ Point_2 $ \\rightarrow$ FT returns the signed distance of two points' $ x$ -coordinates.

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Signed_y_distance_2 = "adaptable binary function class: Point_2 $ \\times$ Point_2 $ \\rightarrow$ FT returns the signed distance of two points' $ y$ -coordinates.

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Infinity_distance_2 = "adaptable binary function class: Point_2 $ \\times$ Point_2 $ \\rightarrow$ FT returns the $ ||\\cdot||_{\\infty}$ distance of two points.

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Signed_infinity_distance_2 = "adaptable binary function class: Point_2 $ \\times$ Point_2 $ \\rightarrow$ FT returns the signed $ ||\\cdot||_{\\infty}$ distance of two points.

Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_point_2_below_left_implicit_point_2 = "3-argument function class: Point_2 $ \\times$ Point_2 $ \\times$ FT $ \\rightarrow$ Point_2 .
...
Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_point_2_below_right_implicit_point_2 = "3-argument function class: Point_2 $ \\times$ Point_2 $ \\times$ FT $ \\rightarrow$ Point_2 .
...
Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_point_2_above_right_implicit_point_2 = "3-argument function class: Point_2 $ \\times$ Point_2 $ \\times$ FT $ \\rightarrow$ Point_2 .
...
Returns: unspecified_type";
constexpr const char* RectangularPCenterTraits_2_Construct_point_2_above_left_implicit_point_2 = "3-argument function class: Point_2 $ \\times$ Point_2 $ \\times$ FT $ \\rightarrow$ Point_2 .
...
#endif // CGAL_PYTHON_DOCSTRINGS_BOUNDING_VOLUMES_H
