...
auto export_property_map_bool(py::module_& m, const std::string& name) {
  using Graph_property_map = typename Graph::template Property_map<Key, bool>;

  m.def("get", [](const Graph_property_map& p, const Key& t) { return py::bool_(get(p, t)); },
        py::arg("property_map"), py::arg("key"),
  ...
  return py::class_<Graph_property_map>(m, name.c_str(), "Boolean graph property map.")
    ...
    .def("__getitem__", [](Graph_property_map& self, Key key) -> bool { return self[key]; },
         py::arg("key"),
         "Returns the boolean value associated with a key.")
    .def("__setitem__", [](Graph_property_map& self, Key key, bool value) { self[key] = value; },
         py::arg("key"), py::arg("value"),
