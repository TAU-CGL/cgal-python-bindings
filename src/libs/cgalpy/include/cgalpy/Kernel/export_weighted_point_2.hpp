...
void export_weighted_point_2(C& c) {
  ...
  c.def(py::init<const Pnt_2&>(),
    ...
    .def("homogeneous",
         [](const Wd_pnt_2& p, int i) -> decltype(auto) {
           if (i < 0 || i >= 3)
             throw py::index_error(
               "Weighted_point_2 homogeneous index out of range");
    ...
    .def("cartesian",
         [](const Wd_pnt_2& p, int i) -> decltype(auto) {
           if (i < 0 || i >= 2)
             throw py::index_error(
               "Weighted_point_2 Cartesian index out of range");
    ...
    .def("__getitem__",
         [](const Wd_pnt_2& p, int i) -> decltype(auto) {
           if (i < 0 || i >= 2)
             throw py::index_error("Weighted_point_2 index out of range");
           return p[i];
         },
         py::arg("i"),
         "returns cartesian(i).")
    .def("dimension", &Wd_pnt_2::dimension,
         "returns the dimension (the constant 2).")
    .def("bbox", &Wd_pnt_2::bbox, "returns a bounding box containing p.")
    .def("transform", &Wd_pnt_2::transform, py::arg("t"),
