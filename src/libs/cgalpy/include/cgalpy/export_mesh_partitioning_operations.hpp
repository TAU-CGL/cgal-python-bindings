...
C define_boost_partitioning_operations(C& c) {
  ...
  c.def("alpha_expansion_graphcut",
        [](const Graph& g, EdgeDoubleMap& edge_cost_map, VertexVectorDoubleMap& vertex_label_cost_map,
           VertexSizeTMap& vertex_label_map, const py::dict& np = py::dict()) {
          // return CGAL::alpha_expansion_graphcut(g, edge_cost_map, vertex_label_cost_map, vertex_label_map);
        ...
        py::arg("g"), py::arg("edge_cost_map"), py::arg("vertex_label_cost_map"), py::arg("vertex_label_map"),
        py::arg("np") = py::dict(),
  ...
  c.def("split_graph_into_polylines",
        [](const Graph& g, Visitor& pv, const std::function<bool(const Vd&, const Graph&)>& is_terminal) {
          struct IsTerminal {
