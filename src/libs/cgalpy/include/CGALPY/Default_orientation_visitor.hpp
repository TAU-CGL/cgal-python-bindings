...
struct Default_orientation_visitor : PMP::Default_orientation_visitor {
  Default_orientation_visitor() : nme(), nmv(), dv(), viipr(), por(), lcp() {}
  ...
  void link_connected_polygons(std::size_t id, const std::vector<std::size_t>& v) { lcp(id, v); }
  ...
  void set_vertex_id_in_polygon_replaced(const std::function<void(std::size_t, std::size_t, std::size_t)>& fn) {
    viipr = fn;
  }
