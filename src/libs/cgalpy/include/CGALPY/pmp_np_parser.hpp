...
template <typename K>
Named_params handle_vertex_principal_curvatures_and_directions(const py::handle& vpcad_handle,
  ...
  for (const auto& item : params) {
    ...
    switch (internal::Hash(key.c_str())) {
      case internal::Hash("allow_move_functor"):
        cgal_parameters =
            cgal_parameters.allow_move_functor(py::cast<std::function<bool(Vd, Point, Point)>>(item.second));
        break;
      case internal::Hash("vertex_principal_curvatures_and_directions"):
        ...
        //   cgal_parameters = cgal_parameters.vertex_point_map(py::cast<typename Mesh::template Property_map<Vd,
        //   Point>>(item.second)); break;
        // case internal::Hash("vertex_is_constrained_map"):
        //   cgal_parameters.vertex_is_constrained_map(py::cast<typename Mesh::template Property_map<Vd,
        //   bool>>(item.second)); break;
        // case internal::Hash("vertex_index_map"):
        //   cgal_parameters = cgal_parameters.vertex_index_map(py::cast<typename Mesh::template Property_map<Vd,
        //   std::size_t>>(item.second)); break;
        // case internal::Hash("face_index_map"):
        //   cgal_parameters = cgal_parameters.face_index_map(py::cast<typename Mesh::template Property_map<Fd,
        //   std::size_t>>(item.second)); break;
        // case internal::Hash("edge_is_constrained_map"):
        //   cgal_parameters = cgal_parameters.edge_is_constrained_map(py::cast<typename Mesh::template Property_map<Ed,
        //   bool>>(item.second)); break;
        // case internal::Hash("halfedge_index_map"):
        //   cgal_parameters = cgal_parameters.halfedge_index_map(py::cast<typename Mesh::template Property_map<Hd,
        //   std::size_t>>(item.second)); break;
        // case internal::Hash("edge_index_map"):
        //   cgal_parameters = cgal_parameters.edge_index_map(py::cast<typename Mesh::template Property_map<Ed,
        //   std::size_t>>(item.second)); break;
        // case internal::Hash("point_to_vertex_map"):
        //   cgal_parameters = cgal_parameters.point_to_vertex_map(py::cast<typename Mesh::template Property_map<Point,
        //   Vd>>(item.second)); break;
        // case internal::Hash("vertex_normal_map"):
        //   cgal_parameters = cgal_parameters.vertex_normal_map(py::cast<typename Mesh::template Property_map<Vd,
        //   Point>>(item.second)); break;
        // case internal::Hash("vertex_incident_patches_map"):
        //   cgal_parameters = cgal_parameters.vertex_incident_patches_map(py::cast<typename Mesh::template
        //   Property_map<Vd, int>>(item.second)); break;
        // // case internal::Hash("face_size_map"): // this can have a different type
        // //   cgal_parameters = cgal_parameters.face_size_map(py::cast<typename Mesh::template Property_map<Fd,
        // std::size_t>>(item.second));
        // //   break;
        // case internal::Hash("face_patch_map"):
        //   cgal_parameters = cgal_parameters.face_patch_map(py::cast<typename Mesh::template Property_map<Fd,
        //   std::size_t>>(item.second)); break;
        // case internal::Hash("vertex_mean_curvature_map"):
        //   cgal_parameters = cgal_parameters.vertex_mean_curvature_map(py::cast<typename Mesh::template
        //   Property_map<Vd, Ft>>(item.second)); break;
