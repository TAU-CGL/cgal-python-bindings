...
struct Polyhedral_envelope : CGAL::Polyhedral_envelope<GeomTraits> {
  ...
  Polyhedral_envelope(TriangleMesh& tmesh, double epsilon, const py::dict& np = py::dict())  // {
      : CGAL::Polyhedral_envelope<GeomTraits>(
            tmesh,
            epsilon,
            internal::parse_pmp_np<TriangleMesh>(np).face_epsilon_map(get_face_prop_map<TriangleMesh, double>(
                tmesh,
                "INTERNAL_MAPA",
  ...
  Polyhedral_envelope(const std::vector<Fd>& face_range,
                      TriangleMesh& tmesh,
                      double epsilon,
  ...
  Polyhedral_envelope(const std::vector<Point_3> points,
