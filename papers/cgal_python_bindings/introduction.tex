...
built.\footnote{Some dependencies of \cgal{} might need to be
  installed in advanced}. On the hand, when running an application
written in Python that depends on \cgal{} the libraries that contain
the bindings must be accessible. Typically, those bindings are
generated ahead of time with little knowledge about the application
itself. This would impose a severe limitation in cases like ours,
where the number of objects to be bound is large. In cases where the
type pf the objects to be bound are closely tied with the application
itself, generating bindings ahead of time is, naturally,
...
presented in Section~\ref{sec:bind-gen}. Modern C++ techniques and
idioms used in the binding implementation are described in
Section~\ref{sec:bind-code}.
