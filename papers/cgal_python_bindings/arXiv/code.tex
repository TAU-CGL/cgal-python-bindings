...
% -----------------------------------------------------------------------------
\subsection{Multi Element Input and Output}
\label{sec:code:range}
% -----------------------------------------------------------------------------
The Standard Template Library (STL) provides various type-safe
containers for storing collections of related objects. The
\cppLstinline{Container} concept describe types of objects that store
...
container. The Boost.Range library provides an alternative modern
method, which uses \emph{ranges}, for applying algorithms on
collections of objects. In particular, it utilizes a new family of
concepts that refine a basic \cppLstinline{Range} concept. The
\cppLstinline{Range} concept is similar to the
\cppLstinline{Container} concept; it requires the provision of
iterators for accessing a half-open range of elements and provides
information about the number of elements in the range. The new method
results in code that is more efficient and more expressive (and thus
more comprehensible).\footnote{See
...
wrapper function that accepts a Python list \cppLstinline{bp::list} as
an argument;\footnote{For more information on Python lists, see, e.g.,
  \url{https://docs.python.org/3/tutorial/datastructures.html\#more-on-lists}.}
the wrapper function calls the original function passing the input
collection properly; see Listing~\ref{lst:insert} for an example of
such a function that uses iterators; the wrapper function is shown in
...
\begin{lstfloat}
  \begin{lstlisting}[style=cppFramedStyle,basicstyle=\normalsize,%
                     label={lst:insert},%
                     caption={The signature of the global function that inserts a collections of curves into an arrangement.}]
...
\begin{lstfloat}
  \begin{lstlisting}[style=pythonFramedStyle,basicstyle=\normalsize,%
                     ...
                     caption={An code sample in Python that inserts three curves into an arrangement.}]
arr = Arrangement_2()
...
\label{sec:code:misc:kdtree}
% -----------------------------------------------------------------------------
The \kdtree{} dimension and the dimension of related classes is
templated and can thus only be decided at compile time via the
\cmake{} variable \myLstinline{CGALPY_SPATIAL_SEARCHING_DIMENSION}.
