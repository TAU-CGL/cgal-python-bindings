...
Python using nothing more than a C++ compiler. It exploits the full
power of C++ metaprogramming techniques using a rich set of features
and high-level user-friendly interface. Boost.Python exploits as much
as information as possible from the source code to be wrapped. This
approach is referred to as \empy{user guided wrapping}. Additional
...
same full-featured C++ language as the code being exposed, which
has the potential of generating efficient binding. In this section
we describe some of the techniques we use to ensure that.
...
\myLstinline{Triangulation\_2} and
\myLstinline{Delaunay\_triangulation\_2}. In those cases we use a
function template (i.e \myLstinline{export_triangulation}) templated
