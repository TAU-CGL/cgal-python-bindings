\tikzset{tableA/.style={
  ...
  nodes={minimum height=2.8em,rectangle,draw=black,align=left,anchor=center},
...
generated library is CGALPY.so. However, the user can override the
name with a string that maps to the set of bound instances. This is
useful when more then one instance of the same template must be
bound. Each execution of cmake followed by make generates a single
library.
% Example: Arrangement_2<Geometry_traits_2, Dcel>
%

The \cgal{} library consists of several packages.  We use the same
partition and to refer to each package as a module. Each module has a
long (and meaningful) name and a short name; see
Table~ref{tab:module-name} forthe list of modules supported so
...
example, the long name of the 2D Arrangement module is
...
the 3D Triangulation packages are \myLstinline{Arr2} and
...
has the suffix \myLstinline{CGALPY_} followed by as many as substrings
...
C\myLstinline{GALPY_kernelEpecInt_Arr2SegPlainPl} of a generated
...
traits class that handles segments and a plain DCEL, and point
location queries.
...
\begin{table}[!]
  \tikzset{table/.style={
      ...
      row sep=-\pgflinewidth,
      column sep=-\pgflinewidth,
      nodes={minimum height=1.6em,rectangle,draw=black,align=left,anchor=center},
      nodes in empty cells,
      every even row/.style={nodes={fill=gray!20}},
...
to cmake. The number of flags is already large, and it is expected to
grow in the future. All flags have the prefix
\myLstinline{CGALPY_}. In the following tables this prefix is omitted.
...
\begin{table}[!htp]
  \centering\begin{tikzpicture}
    \matrix (first) [tableA] {
Name & Type &  Default & Description\\
\myLstinline{ARR2\_GEOMETRY\_TRAITS\_NAME}    & String  & \myLstinline{segment}     & The geometry traits of a 2D arrangement. Options are: linear, segment, nonCachingSegment, conic, algebraic, or circleSegment\\
\myLstinline{ARR2\_DCEL\_NAME}                & String  & \myLstinline{allExtended} & The DCEL of a 2D arrangement. Options are: plain, faceExtended, halfedgeExtended, vertexExtended, allExtended\\
