...
\item The Dcel template-parameter should be substituted with a class that
  ...
  \begin{compactenum}
  \item \myLstinline{Arr_extended_vertex<VertexBase, VertexData>}---extends
    the vertex topological-features of the Dcel with a record of type
    \myLstinline{VertexData}.
    %
  \item \myLstinline{Arr_extended_halfedge<HalfedgeBase, HalfedgeData>}---extends
    the halfedge topological-features of the Dcel with a record of type
    \myLstinline{HalfedgeData}.
    %
  \item \myLstinline{Arr_extended_face<FaceBase, FaceData>}---extends
    the face topological-features of the Dcel with a record of type
    \myLstinline{FaceData}.
    %
  \item \myLstinline{Arr_extended_dcel<Traits, VertexData, HalfedgeData, FaceData, VertexBase, HalfedgeBase, FaceBase>}---extend
    all the topological-features of the Dcel.
  \end{compactenum}
  The \myLstinline{VertexBase} template parameter must be substituted with
  either the base vertex type in the default Dcel, or an extended vertex type.
  Similarly, the \myLstinline{HalfedgeBase} parameter must be substituted with
  the base halfedge type in the default Dcel, or an extended halfedge type,
  and the \myLstinline{FaceBase} parameter must be substituted with the base
  face type in the default Dcel, or an extended face type.

  When the bindings are generated each one of
  the \myLstinline{VertexData}, \myLstinline{HalfedgeData},
  and \myLstinline{FaceData} template parameters must be substituted
  with a known C++ type. Thus, it is impossible to use a custom C++
  type for the substitution. Instead, any one of these parameters is
  substituted with the generic Python
  object \myLstinline{boost::python::object}. This object provides a
  generalized interface to Python objects and increases flexibility.
\end{itemize}
...
\begin{compactenum}
\item \myLstinline{Triangulation_3<Traits_3, DataStructure_3, SpatialLockDataStructure>},
\item \myLstinline{Delaunay_triangulation_3<Traits_3, DataStructure_3, LocationPolicy>}, or
\item \myLstinline{Regular_triangulation_3<Traits_3, DataStructure_3>}
\end{compactenum}
Instance of the \myLstinline{Delaunay_triangulation_3<>} and
\myLstinline{Regular_triangulation_3<>} class template can be used to
...
\myLstinline{Traits_3} must be substituted with the geometric traits
class that provides the type of points to use as well as elementary
operations on them. The type that substitutes the
\myLstinline{DataStructure_3} parameter stores the combinatorial
...
class templates
\begin{compactenum}
\item \myLstinline{Periodic_3_triangulation_3<Traits_3, DataStructure_3>},
\item \myLstinline{Periodic_3_Delaunay_triangulation_3<Traits_3, DataStructure_3>}, and
\item \myLstinline{Periodic_3_regular_triangulation_3<Traits_3, DataStructure_3>}.
\end{compactenum}

%%%%%%%% 3D Triangulation
\begin{table}[!htp]
  \centering\begin{tikzpicture} \matrix (first) [tableB, row
...
\myLstinline{TRI3\_LOCATION\_POLICY\_NAME} & String & \myLstinline{compact}    & The location policy\\
  ...
  \centering\begin{tikzpicture}
    ...
    \myLstinline{plain} & \myLstinline{Triangulation_3<Traits_3, DataStructure_3>}\\
    \myLstinline{regular} & \myLstinline{Regular_triangulation_3<Traits_3, DataStructure_3>}\\
    \myLstinline{delaunay} & \myLstinline{Delaunay_triangulation_3<Traits_3, DataStructure_3>}\\
    \myLstinline{periodicPlain} & \myLstinline{Periodic_3_triangulation_3<Traits_3, DataStructure_3>}\\
    \myLstinline{periodicRegular} & \myLstinline{Periodic_3_regular_triangulation_3<Traits_2, DataStructure_3>}\\
    \myLstinline{periodicDelaunay} & \myLstinline{Periodic_3_Delaunay_triangulation_3<Traits_2, DataStructure_3>}\\
  };
  \end{tikzpicture}
  %
  \centering\begin{tikzpicture}
...
\myLstinline{Alpha_shape_3<Dt,ExactAlphaComparisonTag>} and
\myLstinline{Fixed_alpha_shape_3<Dt>}. Instances of both
templates can be used to represent a large variaty of alpha shapes for
a given set of points. The template parameter
\myLstinline{Dt} must be substituted with an instance of
one of the class templates that can be used to represent a triangulation,
namly:
\begin{compactenum}
\item \myLstinline{Delaunay_triangulation_3},
\item \myLstinline{Regular_triangulation_3},
\item \myLstinline{Periodic_3_Delaunay_triangulation_3}, or
\item \myLstinline{Periodic_3_regular_triangulation_3}.
\end{compactenum}
Note that \myLstinline{Dt::Geom_traits}, \myLstinline{Dt::Vertex}, and
\myLstinline{Dt::Face} must be models the concepts
\myLstinline{AlphaShapeTraits_3}, \myLstinline{AlphaShapeVertex_3},
and \myLstinline{AlphaShapeCell_3}, respectively.
