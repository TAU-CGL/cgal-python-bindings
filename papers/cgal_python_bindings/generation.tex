...
flags \myLstinline{CGALPY\_AOS2\_GEOMETRY\_TRAITS\_NAME} and
\myLstinline{CGALPY\_AOS2\_DCEL\_NAME} that indicates the geometry
traits and the Dcel, respectively, used to instantiate
the \cppLstinline{Arrangement\_2<>} class template; see
...
\begin{table}[!]
  \centering\begin{tikzpicture} \matrix (first) [tableModuleName] {
    ...
    Surfaces & \myLstinline{ARRANGEMENT\_ON\_SURFACE\_2} & \myLstinline{AOS2}\\ 3D
...
packages are \pyLstinline{Arr2} and
\pyLstinline{Tri3}, respectively. Each of these two namespaces, for
example, have the attribute \myLstinline{Vertex\_handle} (i.e.,
\myLstinline{Aos2.Vertex\_handle} and
\myLstinline{Tri3.Vertex\_handle}).

The above is conveniently acchived with the use of the
\cppLstinline{bp::scope} class of Boost.Python. Constructing an object of
...
constructs in C++:\footnote{This example is a simplified version
of the example shown at \url{https://www.boost.org/doc/libs/1_76_0/libs/python/doc/html/reference/high_level_components/boost_python_scope_hpp.html}.}
...
In addition we introduce as many (Python) namespaces as modules, and
nest every exposed name of a bound entity in the appropriate
namespaces according to the module of the entity. For example, assume
that \myLstinline{CGALPY} includes bindings for an instance of the
class template \cppLstinline{Arrangement\_2<GeometryTraits>}, where the
\cppLstinline{GeometryTraits} template parameter is substituted with a
traits class that handles segments.  The following code exerpt
construct a the segment $(0,0),(1,0)$.
...
% -----------------------------------------------------------------------------
By default, the base name of the generated library
is \myLstinline{CGALPY}. However, the user can override the name with
a generated string that maps to the set of bound types. This is
...
base name of the generated library is \myLstinline{CGALPY} or not. If
